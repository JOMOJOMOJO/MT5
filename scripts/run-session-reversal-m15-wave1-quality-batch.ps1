param(
    ...
    [string]$MatrixPath = "reports\backtest\runs\20260708_session_reversal_m15_wave1_quality\run_matrix.csv",
...
$runRoot = Join-Path $repoRoot "reports\backtest\runs\20260708_session_reversal_m15_wave1_quality"
...
Write-BatchLog "Starting M15 wave1 quality batch. terminal=$TerminalPath matrix=$resolvedMatrixPath selected=$($selected.Count) timeout_per_run=$TimeoutSecondsPerRun force=$Force"
...
if ($AnalyzeWhenDone) {
    Write-BatchLog "Running M15 wave1 quality analysis script."
    $analysisOutput = & python (Join-Path $repoRoot "scripts\analyze-session-reversal-m15-wave1-quality-cycles.py") 2>&1
