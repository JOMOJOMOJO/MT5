param(
    ...
    [string]$MatrixPath = "reports\backtest\runs\20260708_session_reversal_nearmiss_separators\run_matrix.csv",
...
$runRoot = Join-Path $repoRoot "reports\backtest\runs\20260708_session_reversal_nearmiss_separators"
...
Write-BatchLog "Starting near-miss separator batch. terminal=$TerminalPath matrix=$resolvedMatrixPath selected=$($selected.Count) timeout_per_run=$TimeoutSecondsPerRun force=$Force"
...
if ($AnalyzeWhenDone) {
    Write-BatchLog "Running near-miss analysis script."
    $analysisOutput = & python (Join-Path $repoRoot "scripts\analyze-session-reversal-nearmiss-separator-cycles.py") 2>&1
