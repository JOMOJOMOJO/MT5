param(
    ...
    [string]$MatrixPath = "reports\backtest\ExpectedValue_MultiCurrency_FXSessionReversalPullbackTrader_timeframe_matrix_run_matrix_2025.csv",
...
$runRoot = Join-Path $repoRoot "reports\backtest\runs\20260628_session_reversal_pullback_timeframe_matrix"
$statusPath = Join-Path $runRoot "batch_status.csv"
$batchLogPath = Join-Path $runRoot ("batch_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
$eaName = "ExpectedValue_MultiCurrency_FXSessionReversalPullbackTrader"
...
Write-BatchLog "Starting matrix batch. terminal=$TerminalPath matrix=$resolvedMatrixPath selected=$($selected.Count) timeout_per_run=$TimeoutSecondsPerRun force=$Force"
...
foreach ($run in $selected) {
    ...
    Write-BatchLog "[$index/$($runs.Count)] RUN run_id=$runId scenario=$scenarioName timeframe=$($run.timeframe_config_id) be=$($run.break_even_id)"
    $started = Get-Date
    $output = @()
    $exitCode = 0
    try {
...
if ($AnalyzeWhenDone) {
    Write-BatchLog "Running analysis script."
    $analysisOutput = & python (Join-Path $repoRoot "scripts\analyze-session-reversal-timeframe-matrix.py") 2>&1
