...
OUT_PREFIX = f"{OUT_BASE}_thirdwave_v3"

SERIES = [
    {
        "period": "2025-02",
        "series_name": "2025_02_thirdwave_v3",
    ...
    {
        "period": "2025-08",
        "series_name": "2025_08_thirdwave_v3",
    ...
    {
        "period": "2025-10",
        "series_name": "2025_10_thirdwave_v3",
    ...
    {
        "period": "2026-Q1",
        "series_name": "2026_q1_thirdwave_v3",
...
def summarize_short_gate(comparison_rows: list[dict[str, object]], all_trades: list[dict[str, object]]) -> tuple[bool, list[str]]:
    ...
    v3_trade_ratio = v3_trades / current_trades if current_trades else 0.0
    ...
    xau_count = sum(1 for t in v3_trades_rows if t["symbol"] == "XAUUSD")
    xau_share = xau_count / len(v3_trades_rows) if v3_trades_rows else 0.0
    long_count = sum(1 for t in v3_trades_rows if t["direction"] == "LONG")
    short_count = sum(1 for t in v3_trades_rows if t["direction"] == "SHORT")
    major_direction_share = max(long_count, short_count) / len(v3_trades_rows) if v3_trades_rows else 0.0
    current_good_labels = sum(1 for t in current_trades_rows if t.get("wave_audit_label") in {"third_wave_initial", "third_wave_middle"})
    v3_good_labels = sum(1 for t in v3_trades_rows if t.get("wave_audit_label") in {"third_wave_initial", "third_wave_middle"})
    current_chasing = sum(1 for t in current_trades_rows if t.get("wave_audit_label") == "chasing_entry")
    v3_chasing = sum(1 for t in v3_trades_rows if t.get("wave_audit_label") == "chasing_entry")
    current_good_share = current_good_labels / len(current_trades_rows) if current_trades_rows else 0.0
    v3_good_share = v3_good_labels / len(v3_trades_rows) if v3_trades_rows else 0.0
    current_chasing_share = current_chasing / len(current_trades_rows) if current_trades_rows else 0.0
    v3_chasing_share = v3_chasing / len(v3_trades_rows) if v3_trades_rows else 0.0
    current_fx = sum(float(t["net_profit"]) for t in current_trades_rows if t["symbol"] != "XAUUSD")
    v3_fx = sum(float(t["net_profit"]) for t in v3_trades_rows if t["symbol"] != "XAUUSD")

    reasons = [
        f"current average PF={current_pf:.3f}, v3 average PF={v3_pf:.3f}",
        f"current average R={current_avg_r:.3f}, v3 average R={v3_avg_r:.3f}",
        f"trade count ratio={v3_trade_ratio:.1%} ({v3_trades}/{current_trades})",
        f"current good-label share={current_good_share:.1%}, v3 good-label share={v3_good_share:.1%}",
        f"current chasing share={current_chasing_share:.1%}, v3 chasing share={v3_chasing_share:.1%}",
        f"current FX net={current_fx:.2f}, v3 FX net={v3_fx:.2f}",
        f"v3 XAUUSD share={xau_share:.1%}",
        f"v3 largest direction share={major_direction_share:.1%}",
    ]
    pass_gate = (
        (v3_pf > current_pf or v3_avg_r > current_avg_r)
        and v3_good_share > current_good_share
        and v3_chasing_share < current_chasing_share
        and (v3_trade_ratio >= 0.50 or (v3_pf > current_pf and v3_avg_r > current_avg_r))
        and v3_fx >= current_fx
        and xau_share < 0.80
...
def write_oos_summary(pass_gate: bool, gate_reasons: list[str]) -> None:
    lines = [
        "# ThirdWave v3 Annual Summary",
        "",
    ]
    if pass_gate:
        lines.extend(
            [
                "Annual validation was permitted by the short-period gate, but this analyzer did not find annual v3 artifacts in the current run set.",
                "Run 2024, 2025, and 2026YTD with `ScenarioSet=V3Comparison` before treating this as an annual result.",
            ]
        )
    else:
        lines.extend(
            [
                ...
                "This follows the requested rule: only run year-level checks if v3 improves entry position without over-filtering or concentrating into one symbol/direction.",
                "",
                "Decision: hold or discard this exact v3 branch. It removes late/chasing entries, but it leaves too few trades and does not improve average PF or average R.",
                "",
                "Gate evidence:",
            ]
        )
...
def write_devlog(pass_gate: bool) -> None:
    path = Path("docs/devlog/2026-06-05-thirdwave-v3-entry-timing.md")
    lines = [
