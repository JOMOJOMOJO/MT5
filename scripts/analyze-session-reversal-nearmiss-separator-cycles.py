...
OUT = REPO / "reports" / "backtest" / "runs" / "20260708_session_reversal_nearmiss_separators"
...
def add_derived_columns(row):
    ...
    row["near_miss_group"] = near_miss_group(row)
...
def write_breakdowns(all_trades):
    breakdowns = {
        "separator_invalidation_quality_breakdown.csv": ["period_id", "variant", "near_miss_group", "m5_invalidation_quality_bucket"],
        "separator_no_immediate_failure_breakdown.csv": ["period_id", "variant", "near_miss_group", "post_break_hold_bucket", "no_immediate_failure_pass"],
        "separator_retest_quality_breakdown.csv": ["period_id", "variant", "near_miss_group", "retest_quality_bucket"],
        "separator_corrective_exhaustion_breakdown.csv": ["period_id", "variant", "near_miss_group", "m5_corrective_exhaustion_bucket"],
        "separator_m15_completion_quality_breakdown.csv": ["period_id", "variant", "near_miss_group", "m15_wave2_completion_quality_bucket", "m15_wave2_depth_bucket"],
        "separator_target_room_breakdown.csv": ["period_id", "variant", "near_miss_group", "target_room_bucket", "clean_path_to_1r", "clean_path_to_1_3r"],
        "separator_granville_quality_breakdown.csv": ["period_id", "variant", "near_miss_group", "sma75_granville_quality_bucket"],
        "mfe_by_separator_mode.csv": ["period_id", "variant", "near_miss_separator_mode", "near_miss_separator_component", "near_miss_separator_pass"],
...
def write_summary(comparison, all_trades, best_single, best_mask):
    ...
    diag = [r for r in all_trades if r.get("run_id") == "full2025_nearmiss_diag"]
    group_a = [r for r in diag if r.get("near_miss_group") == "A_required_light_pass"]
    group_b = [r for r in diag if r.get("near_miss_group") == "B_reject_mfe1_winner"]
    group_c = [r for r in diag if r.get("near_miss_group") == "C_reject_loser_or_no_mfe1"]
    group_d = [r for r in diag if r.get("near_miss_group") == "D_reject_strong_mfe13_winner"]
    separators = [r for r in full if r.get("variant") in SEPARATOR_VARIANTS or r.get("variant") in {"best_single", "best_two"}]
    best_row = max(separators, key=lambda r: (fnum(r.get("avg_r")), fnum(r.get("profit_factor"))), default=None)
    fragments_100 = [r for r in separators if fnum(r.get("trades")) >= 100 and fnum(r.get("avg_r")) > 0 and fnum(r.get("profit_factor")) >= 1.05]
    ...
    answers = [
        ...
        f"- best separator row: `{best_row.get('run_id') if best_row else 'none'}` {fmt(best_row)}",
        f"- analyzer-selected next best single: `{best_single}`, best_two mask: `{best_mask}`.",
        "",
        "## Required Answers",
        f"1. baseline c10 reproduced: {fmt(baseline)}.",
        f"2. required-light reproduced: {fmt(light)}.",
        f"3. broad diagnostic groups: A={len(group_a)}, B={len(group_b)}, C={len(group_c)}, D={len(group_d)}. These are labels inside `full2025_nearmiss_diag`; they do not equal the separate 50-trade required-light run because gating changes entry order and session consumption.",
        f"4. required-light pass vs near-miss winners: see `required_light_pass_vs_nearmiss_winners.csv`; top B invalidation={top_value(group_b, 'm5_invalidation_quality_bucket')}.",
        f"5. required-light pass vs near-miss losers: see `required_light_pass_vs_nearmiss_losers.csv`; top C failure bucket={top_value(group_c, 'post_break_hold_bucket')}.",
        f"6. strong near-miss winners: D={len(group_d)}; see `near_miss_strong_winner_comparison.csv`.",
        f"7. M5 invalidation candle quality separated winners/losers: inspect `separator_invalidation_quality_breakdown.csv`; B top={top_value(group_b, 'm5_invalidation_quality_bucket')}, C top={top_value(group_c, 'm5_invalidation_quality_bucket')}.",
        f"8. no-immediate-failure separated winners/losers: B top={top_value(group_b, 'post_break_hold_bucket')}, C top={top_value(group_c, 'post_break_hold_bucket')}.",
        f"9. retest rejection quality separated winners/losers: B top={top_value(group_b, 'retest_quality_bucket')}, C top={top_value(group_c, 'retest_quality_bucket')}.",
        f"10. corrective exhaustion separated winners/losers: B top={top_value(group_b, 'm5_corrective_exhaustion_bucket')}, C top={top_value(group_c, 'm5_corrective_exhaustion_bucket')}.",
        f"11. M15 wave2 completion quality separated winners/losers: B top={top_value(group_b, 'm15_wave2_completion_quality_bucket')}, C top={top_value(group_c, 'm15_wave2_completion_quality_bucket')}.",
        f"12. target room separated winners/losers: B top={top_value(group_b, 'target_room_bucket')}, C top={top_value(group_c, 'target_room_bucket')}.",
        f"13. 75SMA/Granville separated winners/losers: B top={top_value(group_b, 'sma75_granville_quality_bucket')}, C top={top_value(group_c, 'sma75_granville_quality_bucket')}.",
        "14. separator candidate rows are in `separator_candidate_comparison.csv`.",
        f"15. 100+ positive research fragments: {', '.join(r.get('run_id') for r in fragments_100) or 'none'}.",
        f"16. 200+ shallow-gate candidates: {', '.join(r.get('run_id') for r in gate_pass) or 'none'}.",
...
def main():
    ...
    diag = [r for r in all_trades if r.get("run_id") == "full2025_nearmiss_diag"]
    write_csv(OUT / "near_miss_grouped_trades.csv", diag)
    group_b = [r for r in diag if r.get("near_miss_group") == "B_reject_mfe1_winner"]
    group_c = [r for r in diag if r.get("near_miss_group") == "C_reject_loser_or_no_mfe1"]
    group_d = [r for r in diag if r.get("near_miss_group") == "D_reject_strong_mfe13_winner"]
    group_a_b = [r for r in diag if r.get("near_miss_group") in {"A_required_light_pass", "B_reject_mfe1_winner"}]
    group_a_c = [r for r in diag if r.get("near_miss_group") in {"A_required_light_pass", "C_reject_loser_or_no_mfe1"}]
    features = [
        "m5_invalidation_quality_bucket",
        "post_break_hold_bucket",
        "retest_quality_bucket",
        "m5_corrective_exhaustion_bucket",
        "m15_wave2_completion_quality_bucket",
        "m15_wave2_depth_bucket",
        "target_room_bucket",
        "sma75_granville_quality_bucket",
        "entry_pattern",
        "symbol",
        "direction",
        "session_label",
    ]
    feature_comparison(group_b + group_c, features, "near_miss_winner_loser_feature_comparison.csv")
    feature_comparison(group_d + group_c, features, "near_miss_strong_winner_comparison.csv")
    feature_comparison(group_a_b, features, "required_light_pass_vs_nearmiss_winners.csv")
    feature_comparison(group_a_c, features, "required_light_pass_vs_nearmiss_losers.csv")

    write_breakdowns(all_trades)
    best_single, best_mask = choose_best_separators(comparison)
    write_summary(comparison, all_trades, best_single, best_mask)
