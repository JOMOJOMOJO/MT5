param(
    ...
    [string]$MatrixPath = "reports\backtest\runs\20260709_session_reversal_m15_anchor_bias\run_matrix.csv",
...
$runRoot = Join-Path $repoRoot "reports\backtest\runs\20260709_session_reversal_m15_anchor_bias"
...
Write-BatchLog "Starting M15 anchor bias batch. terminal=$TerminalPath matrix=$resolvedMatrixPath selected=$($selected.Count) timeout_per_run=$TimeoutSecondsPerRun force=$Force"
...
if ($AnalyzeWhenDone) {
    Write-BatchLog "Running M15 anchor bias analysis script."
    $analysisOutput = & python (Join-Path $repoRoot "scripts\analyze-session-reversal-m15-anchor-bias-cycles.py") 2>&1
