...
RUN_ROOT = BACKTEST / "runs" / "20260708_session_reversal_nearmiss_separators"
...
SEPARATOR_MODES = {
    "invalidation": ("invalidation_quality", "2", 1),
    "nofail": ("no_immediate_failure", "3", 2),
    "retest": ("retest_quality", "4", 4),
    "exhaustion": ("corrective_exhaustion", "5", 8),
    "m15completion": ("m15_completion_quality", "6", 16),
    "targetroom": ("target_room", "7", 32),
    "granville": ("granville_quality", "8", 64),
}
...
BASE_VARIANTS = [
    ...
    ("nearmiss_diag", "near_miss_broad_diagnostic", {
        **DIAG_BASE,
        "InpNearMissSeparatorMode": "1",
    }),
]


def separator_values(short):
    _, mode, _ = SEPARATOR_MODES[short]
    return {**DIAG_BASE, "InpNearMissSeparatorMode": mode}


def best_values(best_single, best_mask):
    if best_single in SEPARATOR_MODES:
        single = separator_values(best_single)
    else:
        single = separator_values("invalidation")
    return [
        ("best_single", f"best_single_{best_single}", single),
        ("best_two", f"best_two_mask_{best_mask}", {
            **DIAG_BASE,
            "InpNearMissSeparatorMode": "9",
            "InpNearMissSeparatorCombineMask": str(best_mask),
        }),
        ("one_best_single", f"one_symbol_best_single_{best_single}", {
            **single,
            "InpScenarioMode": "1",
        }),
        ("one_best_two", f"one_symbol_best_two_mask_{best_mask}", {
            **DIAG_BASE,
            "InpScenarioMode": "1",
            "InpNearMissSeparatorMode": "9",
            "InpNearMissSeparatorCombineMask": str(best_mask),
...
def build_variants(best_single, best_mask):
    variants = list(BASE_VARIANTS)
    for short, (name, _, _) in SEPARATOR_MODES.items():
        variants.append((short, f"separator_{name}", separator_values(short)))
    variants.extend(best_values(best_single, best_mask))
    return variants
...
def build_runs(phase, best_single, best_mask):
    variants = build_variants(best_single, best_mask)
    runs = []
    if phase in {"q1", "all"}:
...
def inputs_for(run, magic):
    ...
    values["InpLogFolder"] = f"fx_session_reversal_nearmiss_separator_{run[0]}"
...
def tester_ini(run, preset_name):
    ...
    return "\n".join([
        "; Near-miss separator validation.",
        "",
        "[Experts]",
        "Enabled=0",
        "AllowLiveTrading=0",
...
def main():
    ...
    parser.add_argument("--best-single", default="invalidation", choices=sorted(SEPARATOR_MODES))
    parser.add_argument("--best-mask", type=int, default=1 | 32)
    ...
    rows = []
    for index, run in enumerate(build_runs(args.phase, args.best_single, args.best_mask), start=1):
        run_id, period_id, scenario_key, scenario_mode, scenario_name, from_date, to_date, variant, variant_name, _ = run
        magic = 202607080800 + index
        ...
        for key in [
            ...
            "InpNearMissSeparatorMode", "InpNearMissSeparatorCombineMask",
            "InpExitMode", "InpMaxHoldBars",
        ]:
            row[key] = values[key]
        rows.append(row)
