...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a doctrine-aligned S1 sell-only event study for USDJPY.")
...
def simulate_short_trade(
    ...
    realized_pips = (entry - exit_price) / pip - spread_cost
...
def build_event_frame(df: pd.DataFrame, stop_loss_pips: float, target_r: float, max_hold_bars: int) -> pd.DataFrame:
    ...
    for i in range(max(40, slow_slope_lookback + 10), len(df) - max_hold_bars - 2):
        ...
        transition_line = float(df.at[latest_high, "high"])
        ...
        if not (
            ...
            continue
        if float(df.at[i, "high"]) >= transition_line:
            continue
        ...
        pullback_pips = float((float(df.at[i, "high"]) - counter_sample["low"].min()) / pip)
        if impulse_pips <= 0.0:
            continue
        pullback_ratio = pullback_pips / impulse_pips
        transition_distance_pips = float((transition_line - float(df.at[i, "high"])) / pip)
        ...
        outcome_r, bars_to_exit = simulate_short_trade(df, i, pip, stop_loss_pips, target_r, max_hold_bars)
...
def search_candidates(events: pd.DataFrame, split_date: pd.Timestamp) -> pd.DataFrame:
    if events.empty:
        return pd.DataFrame()
            ...
            for min_transition in (0.0, 1.0, 2.0, 3.0):
                for max_pullback_ratio in (0.35, 0.45, 0.55, 0.70):
                    for min_rej_close in (0.60, 0.65, 0.70, 0.75):
                        for min_impulse in (8.0, 10.0, 12.0):
                            mask = (
                                ...
                                & (events["rejection_close_location"] <= (1.0 - min_rej_close))
                            ...
                            if train_stats["trades"] < 8 or test_stats["trades"] < 4:
                                continue
                            score = (
                                test_stats["expectancy_r"] * 0.55
                                + train_stats["expectancy_r"] * 0.25
                                + min(train_stats["tpd"], test_stats["tpd"]) * 0.12
                                + test_stats["win_rate"] * 0.08
                            )
                            rows.append(
                                CandidateRule(
...
def default_output_dir(symbol: str, timeframe: str) -> Path:
    ...
    return DEFAULT_OUTPUT_ROOT / f"{timestamp}-{slugify(symbol)}-{timeframe.lower()}-golden-s1-event-study"
...
def write_outputs(output_dir: Path, events: pd.DataFrame, candidates: pd.DataFrame, metadata: dict[str, Any]) -> None:
    ...
    lines = [
        "# USDJPY Golden S1 Sell-Only Event Study",
        "",
    ...
    if candidates.empty:
        ...
        for _, row in candidates.head(10).iterrows():
            lines.append(
                ...
                f"impulse>={row['min_impulse_pips']:.1f}: train `{row['train_trades']} trades, {row['train_tpd']:.2f}/day, exp {row['train_expectancy_r']:.3f}R`, "
...
def main() -> int:
    ...
    candidates = search_candidates(events, split_date)
