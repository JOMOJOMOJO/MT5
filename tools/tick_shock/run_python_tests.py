...
def main() -> int:
    ...
    parser.add_argument("--phase", choices=("pre-fix", "post-fix", "step10", "step11", "step14r", "step15a", "step15f", "step15g"), default="post-fix")
    ...
    log_path = root / "reports" / "tests" / "tick_shock" / ("step14r_final/python_tests.log" if args.phase == "step14r" else ("step15a_green/python_tests.log" if args.phase == "step15a" else ("step15f_green/python_tests.log" if args.phase == "step15f" else ("step15g_green/python_tests.log" if args.phase == "step15g" else f"{step}_python_tests.log"))))
    ...
    raw_name = "raw" if args.phase == "pre-fix" else ("step10_raw" if args.phase == "step10" else ("step11_raw" if args.phase == "step11" else ("step14r_final/raw" if args.phase == "step14r" else ("step15a_green/raw" if args.phase == "step15a" else ("step15f_green/raw" if args.phase == "step15f" else ("step15g_green/raw" if args.phase == "step15g" else "step12_raw"))))))
    ...
    filename = "step05_pre_fix_results.csv" if args.phase == "pre-fix" else ("step10_post_refactor_results.csv" if args.phase == "step10" else ("step11_pre_fix_results.csv" if args.phase == "step11" else ("step14r_final/results.csv" if args.phase == "step14r" else ("step15a_green/suite_results.csv" if args.phase == "step15a" else ("step15f_green/suite_results.csv" if args.phase == "step15f" else ("step15g_green/suite_results.csv" if args.phase == "step15g" else "step12_post_fix_results.csv"))))))
    ...
    print(result_path)
    if args.phase in ("post-fix", "step14r", "step15a", "step15f", "step15g") and any(counts[key] for key in ("FAIL", "XFAIL", "XPASS")):
        return 1
    return 0
