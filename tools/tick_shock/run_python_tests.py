...
def main() -> int:
    ...
    parser.add_argument("--phase", choices=("pre-fix", "post-fix", "step10"), default="post-fix")
    args = parser.parse_args()
    root = args.repo_root.resolve()
    tests_dir = root / "tests" / "tick_shock" / "python"
    step = "step05" if args.phase == "pre-fix" else ("step10" if args.phase == "step10" else "step06")
    ...
    raw_name = "raw" if args.phase == "pre-fix" else ("step10_raw" if args.phase == "step10" else "step06_raw")
    ...
    filename = "step05_pre_fix_results.csv" if args.phase == "pre-fix" else ("step10_post_refactor_results.csv" if args.phase == "step10" else "step06_post_fix_results.csv")
