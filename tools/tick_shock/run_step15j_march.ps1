...
$presetName="post_shock_excursion.set";$presetPath=Join-Path $runDir $presetName
$preset=@(
...
"InpSchemaVersion=tickshock-event-v1+tail-v1-persistent+post-shock-excursion-v1","InpEnableDebug=false","InpDebugSymbol=EURUSD","InpDebugMaxMessages=200")
...
[IO.File]::WriteAllText((Join-Path $runDir "run_command.txt"),"powershell -NoProfile -ExecutionPolicy Bypass -File tools/tick_shock/run_step15j_march.ps1 -TimeoutSeconds $TimeoutSeconds`r`n",[Text.UTF8Encoding]::new($false))
...
$suffixes=@("trades.csv","summary.csv","symbol_specs.csv","detector_features.csv","medium_horizon_episode_summary.csv","post_shock_excursion.csv")
