...
$folder="20260829_ts15d_tail_v1_persistent_state_response_202503_r3"
$runId="ts15d_state_202503_r3"
...
$log=Join-Path $root "reports\compile\tick_shock\step15d_march_research_ea.log"
...
"InpSchemaVersion=tickshock-event-v1+detector-feature-v2+control-v1+funnel-v1+event-response-v1+state-conditioned-response-v1","InpEnableDebug=false","InpDebugSymbol=EURUSD","InpDebugMaxMessages=200")
...
[IO.File]::WriteAllText((Join-Path $runDir "run_command.txt"),"powershell -NoProfile -ExecutionPolicy Bypass -File tools/tick_shock/run_step15d_march.ps1 -TimeoutSeconds $TimeoutSeconds`r`n",[Text.UTF8Encoding]::new($false))
