param([int]$TimeoutSeconds=3000)
$ErrorActionPreference="Stop"
$root=(Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path
$folder="20260904_ts15n_delayed_decision_r1_202503";$runId="ts15n_delayed_decision_r1_202503";$runDir=Join-Path $root "reports\backtest\runs\$folder"
if(Test-Path -LiteralPath $runDir){throw "Refusing to overwrite $runDir"}
New-Item -ItemType Directory -Path $runDir|Out-Null
$source=Join-Path $root "mql\Experts\ExpectedValue_MultiCurrency_TickShockResearch.mq5";$log=Join-Path $root "reports\compile\tick_shock\step15n_march_research_ea.log"
...
$presetName="post_shock_delayed_decision.set";$presetPath=Join-Path $runDir $presetName
$preset=@(
...
"InpSchemaVersion=tickshock-event-v1+tail-v1-persistent+post-shock-excursion-v2+clean-move-causal-feature-v1+delayed-decision-v1","InpEnableDebug=false","InpDebugSymbol=EURUSD","InpDebugMaxMessages=200")
...
[IO.File]::WriteAllLines($configPath,$config,[Text.UTF8Encoding]::new($false));Copy-Item $ex5 (Join-Path $runDir "executed_EA.ex5");Copy-Item $log (Join-Path $runDir "compile.log")
[IO.File]::WriteAllText((Join-Path $runDir "run_command.txt"),"powershell -NoProfile -ExecutionPolicy Bypass -File tools/tick_shock/run_step15n_march.ps1 -TimeoutSeconds $TimeoutSeconds`r`n",[Text.UTF8Encoding]::new($false))
...
$suffixes=@("trades.csv","summary.csv","symbol_specs.csv","detector_features.csv","medium_horizon_episode_summary.csv","detection_time_snapshots.csv","post_shock_excursion.csv","clean_move_causal_features.csv","delayed_decision_checkpoints.csv","delayed_decision_actions.csv")
...
$hashRows=@("source_commit=$commit","source_sha256=$((Get-FileHash $source -Algorithm SHA256).Hash)","delayed_module_sha256=$((Get-FileHash (Join-Path $root 'mql\Include\TickShock\TickShockDelayedDecision.mqh') -Algorithm SHA256).Hash)","ex5_sha256=$ex5Hash","terminal_path=$terminalExe","terminal_sha256=$((Get-FileHash $terminalExe -Algorithm SHA256).Hash)")
[IO.File]::WriteAllLines((Join-Path $runDir "source_hashes.txt"),$hashRows,[Text.UTF8Encoding]::new($false));Write-Host "Completed $runDir"
