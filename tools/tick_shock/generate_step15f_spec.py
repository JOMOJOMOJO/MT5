...
def main():
    ...
    for group,num,field,value,unit in CASES:
        ...
        with (fixtures/f'{tid}_config.csv').open('w',encoding='utf-8',newline='') as h:
            w=csv.writer(h);w.writerow(('key','value','unit','note'));w.writerows((('case',tid,'id','frozen'),('feature_spec_hash',fh,'sha256','canonical registry'),('model_family_hash',mh,'sha256','finite family'),('seed',20260831,'seed','deterministic'),('production_formula_used_for_expected','false','bool','independent oracle'),('production_function_used_for_expected','false','bool','independent oracle'),('oracle_source','docs/research/tick_shock/03_test_oracle_calculation.md','path','independent oracle contract')))
