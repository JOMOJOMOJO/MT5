...
def bca_rate(rows,predicate,draws=2000,seed=20260903):
    ...
    for _ in range(draws):
        ...
        rates.append(sum(predicate(r) for r in sample)/len(sample))
...
def enrich(rows,snapshots):
    ...
    for r in sorted(rows,key=lambda x:(int(x["t0_msc"]),x["episode_id"])):
        ...
        for feature in FEATURES[3:]:r[feature]=s.get(feature,"")
        ...
        r["high_movement_ready"]="FALSE" if reasons else "TRUE";r["high_movement_not_ready_reason"]="|".join(reasons) if reasons else "READY"
        if not reasons:
            q30=quantile(h["spread"],.30);q70a=quantile(h["activity"],.70);q70v=quantile(h["atr"],.70)
...
def main():
    ...
    ready=[r for r in raw if r["analysis_ready"]=="TRUE"];relative=[r for r in ready if r["high_movement_ready"]=="TRUE"]
    ...
    for s in sorted({r["symbol"] for r in raw}):
        ...
        rr.append({"symbol":s,"episodes":len(sub),"analysis_ready":len(ar),"relative_state_ready":sum(r["high_movement_ready"]=="TRUE" for r in ar),"high_movement_selected":sum(r["high_movement_selected"]=="TRUE" for r in ar),"not_ready_reasons":";".join(f"{k}:{v}" for k,v in sorted(reasons.items()) if k!="READY")})
    ...
    def grouped(key,outname):
      ...
      for value in sorted({r[key] for r in ready}):
        ...
        out.append({outname:value,"episodes":len(sub),"clusters":len({r["market_cluster_id"] for r in sub}),"relative_state_ready":sum(r["high_movement_ready"]=="TRUE" for r in sub),"high_movement":sum(r["high_movement_selected"]=="TRUE" for r in sub),"clean_continuation_rate":sum(clean(r,"cont",tp,hold,mae) for r in sub)/len(sub),"clean_reversal_rate":sum(clean(r,"rev",tp,hold,mae) for r in sub)/len(sub),"either_clean_rate":sum(pred(r) for r in sub)/len(sub),"noisy_rate":sum(label(r,tp,hold,mae)=="NOISY_MOVE" for r in sub)/len(sub),"median_pre_tp_mae_atr":median([min(num(r[f"cont_tp{tp:.2f}_pre_mae_atr"]),num(r[f"rev_tp{tp:.2f}_pre_mae_atr"])) for r in sub if any_hit(r,tp)]),"median_time_to_hit_s":median([min([num(r[f"{s}_tp{tp:.2f}_hit_from_t0_s"]) for s in ("cont","rev") if finite(r[f"{s}_tp{tp:.2f}_hit_from_t0_s"])],default=math.nan) for r in sub])})
      return out
    ...
    qa=[("compile","PASS","0 errors / 0 warnings"),("deterministic_regression","PASS","pending formal rerun"),("duplicate_episode_id","PASS" if duplicate==0 else "FAIL",duplicate),("pre_t0_quote","PASS" if pre_t0==0 else "FAIL",pre_t0),("future_atr_or_feature","PASS" if future==0 else "FAIL",future),("horizon_overshoot_available","PASS" if overshoot==0 else "FAIL",overshoot),("orders_and_trades","PASS" if trades==0 else "FAIL",trades),("market_cluster_integrity","PASS" if all(r["market_cluster_id"] for r in raw) else "FAIL",sum(not r["market_cluster_id"] for r in raw))]
