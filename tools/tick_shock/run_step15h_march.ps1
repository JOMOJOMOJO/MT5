...
$folder="20260902_ts15h_detection_time_continuation_r1_202503"
...
$log=Join-Path $root "reports\compile\tick_shock\step15h_march_research_ea.log"
...
$presetName="detection_time_continuation.set";$presetPath=Join-Path $runDir $presetName
$preset=@(
...
"InpSchemaVersion=tickshock-event-v1+detector-feature-v2+control-v1+funnel-v1+event-response-v1+state-conditioned-response-v1+medium-horizon-response-v1+context-feature-v1+economic-path-v1+detection-time-continuation-v1","InpEnableDebug=false","InpDebugSymbol=EURUSD","InpDebugMaxMessages=200")
...
[IO.File]::WriteAllText((Join-Path $runDir "run_command.txt"),"powershell -NoProfile -ExecutionPolicy Bypass -File tools/tick_shock/run_step15h_march.ps1 -TimeoutSeconds $TimeoutSeconds`r`n",[Text.UTF8Encoding]::new($false))
