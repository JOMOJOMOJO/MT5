...
def journal_evidence():
    ...
    keep=[line for line in text.splitlines() if any(token in line for token in ('InpRunId=ts15e_medium_horizon_202503','real ticks discarded','tick prices mismatch','tick volumes not matched','last prices absent','Test passed in','total ticks for all symbols','memory used','deinitialized reason='))]
