...
$folder="20260901_ts15g_economic_path_r2_202503"
$runId="ts15g_economic_path_r2_202503"
...
$log=Join-Path $root "reports\compile\tick_shock\step15g_march_research_ea.log"
...
$presetName="economic_path.set";$presetPath=Join-Path $runDir $presetName
$preset=@(
...
"InpSchemaVersion=tickshock-event-v1+detector-feature-v2+control-v1+funnel-v1+event-response-v1+state-conditioned-response-v1+medium-horizon-response-v1+context-feature-v1+economic-path-v1","InpEnableDebug=false","InpDebugSymbol=EURUSD","InpDebugMaxMessages=200")
...
[IO.File]::WriteAllText((Join-Path $runDir "run_command.txt"),"powershell -NoProfile -ExecutionPolicy Bypass -File tools/tick_shock/run_step15g_march.ps1 -TimeoutSeconds $TimeoutSeconds`r`n",[Text.UTF8Encoding]::new($false))
...
$hashRows=@("source_commit=$commit","source_sha256=$((Get-FileHash $source -Algorithm SHA256).Hash)","ex5_sha256=$ex5Hash","terminal_path=$terminalExe","terminal_sha256=$((Get-FileHash $terminalExe -Algorithm SHA256).Hash)","feature_spec_sha256=074C40B21F804CEDB414FA0C75DD1A101B7DF808F6254000B641C134C282B597","label_spec_sha256=0DDA57592D4B9067AD6CBC4386C257A1B4F6D8B81B7BFF6188B2919D1D10E072")
