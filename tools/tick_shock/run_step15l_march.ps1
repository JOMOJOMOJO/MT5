...
$folder="20260903_ts15l_clean_move_ml_r1_202503";$runId="ts15l_clean_move_ml_r1_202503";$runDir=Join-Path $root "reports\backtest\runs\$folder"
if(Test-Path -LiteralPath $runDir){throw "Refusing to overwrite $runDir"}
New-Item -ItemType Directory -Path $runDir|Out-Null
$source=Join-Path $root "mql\Experts\ExpectedValue_MultiCurrency_TickShockResearch.mq5";$log=Join-Path $root "reports\compile\tick_shock\step15l_march_research_ea.log"
...
$presetName="clean_move_ml.set";$presetPath=Join-Path $runDir $presetName
$preset=@(
...
"InpSchemaVersion=tickshock-event-v1+tail-v1-persistent+post-shock-excursion-v2+clean-move-causal-feature-v1","InpEnableDebug=false","InpDebugSymbol=EURUSD","InpDebugMaxMessages=200")
...
[IO.File]::WriteAllText((Join-Path $runDir "run_command.txt"),"powershell -NoProfile -ExecutionPolicy Bypass -File tools/tick_shock/run_step15l_march.ps1 -TimeoutSeconds $TimeoutSeconds`r`n",[Text.UTF8Encoding]::new($false))
...
$suffixes=@("trades.csv","summary.csv","symbol_specs.csv","detector_features.csv","medium_horizon_episode_summary.csv","detection_time_snapshots.csv","post_shock_excursion.csv","clean_move_causal_features.csv")
