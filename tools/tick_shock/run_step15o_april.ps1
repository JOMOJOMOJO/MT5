...
$log=Join-Path $root "reports\compile\tick_shock\step15o_formal_research_ea.log"
& (Join-Path $root "scripts\compile.ps1") -Source $source -LogPath $log
$ex5=[IO.Path]::ChangeExtension($source,".ex5")
$commit=(git -C $root rev-parse HEAD).Trim()
$ex5Hash=(Get-FileHash $ex5 -Algorithm SHA256).Hash
$presetName="cross_fx_lead_lag.set";$presetPath=Join-Path $runDir $presetName
$preset=@(
...
"InpSchemaVersion=tickshock-event-v1+tail-v1-persistent+cross-fx-lead-lag-v1","InpEnableDebug=false","InpDebugSymbol=EURUSD","InpDebugMaxMessages=200")
...
[IO.File]::WriteAllText((Join-Path $runDir "run_command.txt"),"powershell -NoProfile -ExecutionPolicy Bypass -File tools/tick_shock/run_step15o_april.ps1 -TimeoutSeconds $TimeoutSeconds`r`n",[Text.UTF8Encoding]::new($false))
...
$suffixes=@("trades.csv","summary.csv","symbol_specs.csv","detector_features.csv","medium_horizon_episode_summary.csv","detection_time_snapshots.csv","post_shock_excursion.csv","clean_move_causal_features.csv","cross_fx_features.csv","cross_fx_feature_freshness.csv","cross_fx_actions.csv")
