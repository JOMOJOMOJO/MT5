param(
  ...
  [string]$Folder="20260908_ts15p_symmetric_oco_micro_profit_202504",
  [string]$RunId="ts15p_symmetric_oco_micro_profit_202504"
)
...
$compile=Join-Path $root "reports\compile\tick_shock\step15p_formal_research_ea.log"
...
$presetName="symmetric_oco_micro_profit.set";$presetPath=Join-Path $runDir $presetName
$preset=@(
...
"InpRewardRisk=1.2","InpMaxHoldSeconds=120","InpShadowSlippageTicks=1.0","InpShadowExitSlippageTicks=1.0","InpCommissionPerLotRoundTurn=0.0","InpCommissionSource=STEP14R_TESTER_DEAL_FIELDS_OBSERVED_ZERO_EURUSD_ONLY",
...
"InpResumeRun=false","InpResumeCheckpoint=","InpResumeLastEventSequence=-1","InpResumeCursorMsc=0","InpResearchPeriod=2025-04-01_TO_2025-05-01","InpTesterModel=REAL_TICKS_MODEL_4","InpSourceCommit=$commit","InpEx5Hash=$ex5Hash",
"InpSchemaVersion=tickshock-event-v1+tail-v1-persistent+symmetric-oco-v1","InpEnableDebug=false","InpDebugSymbol=EURUSD","InpDebugMaxMessages=200")
...
$suffixes=@("trades.csv","summary.csv","symbol_specs.csv","detector_features.csv","medium_horizon_episode_summary.csv","symmetric_oco_scenarios.csv")
...
$terminalExe="C:\Program Files\XMTrading MT5\terminal64.exe";if(-not(Test-Path $terminalExe)){$terminalExe="C:\Program Files\XMTrading MT5 - 2\terminal64.exe"};if(-not(Test-Path $terminalExe)){throw "Missing tester terminal"}
$hashes=@("source_commit=$commit","source_sha256=$((Get-FileHash $source -Algorithm SHA256).Hash)","oco_module_sha256=$((Get-FileHash (Join-Path $root 'mql\Include\TickShock\TickShockSymmetricOco.mqh') -Algorithm SHA256).Hash)","ex5_sha256=$ex5Hash","terminal_path=$terminalExe","terminal_sha256=$((Get-FileHash $terminalExe -Algorithm SHA256).Hash)")
[IO.File]::WriteAllLines((Join-Path $runDir "source_hashes.txt"),$hashes,[Text.UTF8Encoding]::new($false))
& python (Join-Path $root "tools\tick_shock\prepare_step15p_evidence.py") --run-dir $runDir --run-id $runId
