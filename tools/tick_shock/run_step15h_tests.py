...
def main():
    ...
    cases=[r for r in read(ROOT/'tests/tick_shock/spec/test_cases.csv') if r['test_id'].startswith('TS15H-')];raw={r['test_id']:r for r in read(a.raw)} if a.raw and a.raw.exists() else {};out=[]
    for c in cases:
        tid=c['test_id'];e=read(ROOT/c['expected_path'])[0];expected=f"{e['field']}={e['expected_value']}";obs=raw.get(tid)
        if a.phase=='red':status='XFAIL';actual='PRODUCTION_DETECTION_TIME_API_ABSENT';difference='production API absent'
    ...
    folder=ROOT/f'reports/tests/tick_shock/step15h_{a.phase}';folder.mkdir(parents=True,exist_ok=True);counts=Counter(r['status'] for r in out)
    with (folder/f'step15h_{a.phase}_results.csv').open('w',encoding='utf-8',newline='') as h:w=csv.DictWriter(h,fieldnames=FIELDS);w.writeheader();w.writerows(out)
    (folder/f'step15h_{a.phase}_report.md').write_text('# Step 15H '+a.phase.upper()+' report\n\n'+'\n'.join(f'- {x}: {counts[x]}' for x in ('PASS','FAIL','XFAIL','XPASS','SKIP','BLOCKED'))+'\n',encoding='utf-8')
