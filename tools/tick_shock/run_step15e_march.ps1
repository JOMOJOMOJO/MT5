...
$folder="20260830_ts15e_tail_v1_persistent_medium_horizon_202503"
...
$log=Join-Path $root "reports\compile\tick_shock\step15e_march_research_ea.log"
...
$presetName="persistent_medium_horizon.set";$presetPath=Join-Path $runDir $presetName
$preset=@(
...
"InpSchemaVersion=tickshock-event-v1+detector-feature-v2+control-v1+funnel-v1+event-response-v1+state-conditioned-response-v1+medium-horizon-response-v1","InpEnableDebug=false","InpDebugSymbol=EURUSD","InpDebugMaxMessages=200")
...
[IO.File]::WriteAllText((Join-Path $runDir "run_command.txt"),"powershell -NoProfile -ExecutionPolicy Bypass -File tools/tick_shock/run_step15e_march.ps1 -TimeoutSeconds $TimeoutSeconds`r`n",[Text.UTF8Encoding]::new($false))
...
$hashRows=@("source_commit=$commit","source_sha256=$((Get-FileHash $source -Algorithm SHA256).Hash)","ex5_sha256=$ex5Hash","terminal_path=$terminalExe","terminal_sha256=$((Get-FileHash $terminalExe -Algorithm SHA256).Hash)")
