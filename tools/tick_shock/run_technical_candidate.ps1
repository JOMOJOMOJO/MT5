param(
  ...
  [string]$Folder="20260909_tstech_candidate_202504",
  [string]$RunId="tstech_candidate_202504"
)
...
$source=Join-Path $root "mql\Experts\ExpectedValue_MultiCurrency_TickShockTechnicalCandidate.mq5"
...
$baseline=Join-Path $root 'reports\backtest\runs\20260909_tstech_discovery_r2_202504\technical_discovery.set'
$preset=Get-Content -LiteralPath $baseline | ForEach-Object {
  ...
  elseif($_ -match '^InpSchemaVersion='){'InpSchemaVersion=tickshock-event-v1+tail-v1-persistent+technical-discovery-v1+candidate-v1'}
  else{$_}
}
$preset+=@('InpTechnicalRiskMoney=10.0','InpTechnicalMagic=260909731')
...
'Expert=dev\mql\Experts\ExpectedValue_MultiCurrency_TickShockTechnicalCandidate.ex5',
...
[IO.File]::WriteAllText((Join-Path $run 'run_command.txt'),"powershell -NoProfile -ExecutionPolicy Bypass -File tools/tick_shock/run_technical_candidate.ps1 -TimeoutSeconds $TimeoutSeconds -Folder $Folder -RunId $RunId`r`n",[Text.UTF8Encoding]::new($false))
...
foreach($name in @('signals.csv','actions.csv','trades.csv')){
 $path=Join-Path $common ('candidate_'+$name);if(-not(Test-Path -LiteralPath $path)){throw "Missing $path"}
 Copy-Item -LiteralPath $path -Destination (Join-Path $run ('candidate_'+$name))
}
