...
struct TSRSymbolContext
  {
   ...
   TickShock15NPool delayed_decision;
   TickShock15PPool symmetric_oco;
...
int g_delayed_checkpoint_file = INVALID_HANDLE;
int g_delayed_action_file = INVALID_HANDLE;
int g_cross_fx_feature_file = INVALID_HANDLE;
int g_cross_fx_freshness_file = INVALID_HANDLE;
int g_cross_fx_action_file = INVALID_HANDLE;
int g_symmetric_oco_file = INVALID_HANDLE;
...
long g_delayed_checkpoint_rows = 0;
long g_delayed_action_rows = 0;
long g_cross_fx_feature_rows = 0;
long g_cross_fx_freshness_rows = 0;
long g_cross_fx_action_rows = 0;
long g_symmetric_oco_rows = 0;
...
TickShock15OQuoteState g_cross_fx_quotes[];
TickShock15OPool g_cross_fx_pools[];
...
string TSR15JEpisodeHeader()
  {
   ...
   for(int i=0;i<TS15J_HORIZONS;++i)h+=StringFormat(",h%d_target_msc,h%d_quote_msc,h%d_lag_ms,h%d_status,h%d_cont_mfe,h%d_cont_mae,h%d_rev_mfe,h%d_rev_mae",TS15J_HORIZON_SECONDS[i],TS15J_HORIZON_SECONDS[i],TS15J_HORIZON_SECONDS[i],TS15J_HORIZON_SECONDS[i],TS15J_HORIZON_SECONDS[i],TS15J_HORIZON_SECONDS[i],TS15J_HORIZON_SECONDS[i],TS15J_HORIZON_SECONDS[i]);
...
string TSR15NCheckpointHeader()
  {
   return "episode_id,event_id,market_cluster_id,symbol,shock_direction,t0_msc,checkpoint_seconds,checkpoint_target_msc,status,decision_quote_msc,decision_processing_msc,feature_max_source_msc,atr_source_msc,checkpoint_quote_lag_ms,decision_bid,decision_ask,decision_mid,atr_t0,atr_decision,postshock_return_atr,postshock_mfe_to_decision_atr,postshock_mae_to_decision_atr,postshock_range_atr,retracement_from_peak_pct,retracement_from_trough_pct,current_location_in_postshock_range,new_extreme_count,origin_recross_count,time_since_last_extreme_ms,distance_from_last_extreme_atr,net_move_over_path_length,shock_direction_tick_ratio,direction_consistency,realized_abs_move_atr,realized_volatility,recent_5s_acceleration,recent_10s_acceleration,peak_update_interval_ms,extreme_update_rate,path_contraction_ratio,path_expansion_ratio,decision_spread_atr,spread_vs_t0_ratio,spread_vs_postshock_mean,spread_contraction_from_t0,tick_count_postshock,tick_rate_recent_5s,tick_rate_recent_10s,tick_activity_vs_t0,activity_decay,activity_acceleration,schema_version";
  }

string TSR15NActionHeader()
  {
   return "episode_id,event_id,market_cluster_id,symbol,shock_direction,t0_msc,checkpoint_seconds,checkpoint_status,action,action_direction,decision_quote_msc,decision_processing_msc,feature_max_source_msc,entry_eligible_msc,entry_quote_msc,entry_processing_msc,decision_to_entry_lag_ms,entry_bid,entry_ask,entry_price,atr_decision,risk_distance,tp_distance,sl,tp,exit_msc,exit_price,result,realized_r,mfe_r,mae_r,commission_r_0,net_r_commission_002,net_r_commission_005,net_r_commission_010,primary_deadline_msc,schema_version";
  }

string TSR15OFeatureHeader()
  {
   string h="episode_id,event_id,market_cluster_id,symbol,shock_direction,target_usd_sign,t0_msc,t0_quote_msc,t0_processing_msc,atr14_m5,atr_source_msc,status,status_reason";
   for(int w=0;w<TS15O_WINDOWS;++w){string x=IntegerToString(TS15O_WINDOW_SECONDS[w]);h+=",target_usd_return_atr_"+x+"s,valid_cross_symbols_"+x+"s,usd_breadth_count_"+x+"s,usd_breadth_"+x+"s,usd_consensus_median_"+x+"s,shock_aligned_usd_consensus_"+x+"s,target_residual_"+x+"s,shock_aligned_target_residual_"+x+"s";}
   return h+",median_cross_quote_age_ms,max_cross_quote_age_ms,num_prior_cross_fx_movers,first_cross_fx_lead_ms,median_cross_fx_lead_ms,target_leader_rank,target_lead_bucket,h1,h2,h3,h4,future_sources,feature_spec_sha256,schema_version";
  }

string TSR15OFreshnessHeader()
  {return "episode_id,event_id,market_cluster_id,target_symbol,source_symbol,window_seconds,valid,reason,current_quote_msc,current_processing_msc,current_mid,anchor_quote_msc,anchor_processing_msc,anchor_mid,quote_age_ms,anchor_age_ms,atr14_m5,atr_source_msc,usd_orientation,usd_return_atr,crossing_msc,feature_spec_sha256,schema_version";}

string TSR15OActionHeader()
  {return "episode_id,event_id,market_cluster_id,symbol,shock_direction,t0_msc,feature_status,h1,h2,h3,h4,action,action_direction,entry_eligible_msc,entry_quote_msc,entry_processing_msc,entry_bid,entry_ask,entry_price,atr14_m5,risk_distance,tp_distance,sl,tp,exit_msc,exit_price,result,gross_r,net_r_commission_002,net_r_commission_005,net_r_commission_010,mfe_r,mae_r,deadline_msc,feature_spec_sha256,schema_version";}

string TSR15PHeader()
  {
   return "episode_id,event_id,market_cluster_id,symbol,shock_direction,t0_msc,t0_quote_msc,t0_processing_msc,t0_bid,t0_ask,t0_mid,atr14_m5,atr_source_msc,point,pip_size,offset_index,offset_atr,upper_trigger,lower_trigger,entry_status,oco_direction,trigger_msc,trigger_bid,trigger_ask,entry_msc,entry_processing_msc,entry_bid,entry_ask,entry_price,entry_spread,entry_spread_pips,deadline_msc,tp_index,tp_atr,sl_index,sl_atr,tp_price,sl_price,result,exit_msc,exit_bid,exit_ask,exit_price,gross_price,gross_pips,gross_r,mfe_price,mae_price,quote_groups,same_msc_ticks,future_reads,backdates,fallback_quotes,ambiguous_triggers,record_invalid,record_censored,schema_version";
  }
...
bool TSROpenLogs()
  {
   ...
   g_delayed_checkpoint_file = TSROpenCsv("delayed_decision_checkpoints",TSR15NCheckpointHeader());
   g_delayed_action_file = TSROpenCsv("delayed_decision_actions",TSR15NActionHeader());
   g_cross_fx_feature_file = TSROpenCsv("cross_fx_features",TSR15OFeatureHeader());
   g_cross_fx_freshness_file = TSROpenCsv("cross_fx_feature_freshness",TSR15OFreshnessHeader());
   g_cross_fx_action_file = TSROpenCsv("cross_fx_actions",TSR15OActionHeader());
   g_symmetric_oco_file = TSROpenCsv("symmetric_oco_scenarios",TSR15PHeader());
   return g_event_file!=INVALID_HANDLE && g_trade_file!=INVALID_HANDLE && g_summary_file!=INVALID_HANDLE && g_specs_file!=INVALID_HANDLE && g_features_file!=INVALID_HANDLE &&
          ...
          g_medium_episode_file!=INVALID_HANDLE && g_medium_response_file!=INVALID_HANDLE && g_medium_entry_file!=INVALID_HANDLE &&
           ...
           g_delayed_checkpoint_file!=INVALID_HANDLE && g_delayed_action_file!=INVALID_HANDLE && g_cross_fx_feature_file!=INVALID_HANDLE &&
           g_cross_fx_freshness_file!=INVALID_HANDLE && g_cross_fx_action_file!=INVALID_HANDLE && g_symmetric_oco_file!=INVALID_HANDLE;
  }
...
void TSRCloseLogs()
  {
   ...
   TSMt5Close(g_delayed_checkpoint_file);
   TSMt5Close(g_delayed_action_file);
   TSMt5Close(g_cross_fx_feature_file);
   TSMt5Close(g_cross_fx_freshness_file);
   TSMt5Close(g_cross_fx_action_file);
   TSMt5Close(g_symmetric_oco_file);
  }
...
bool TSRInitializeSymbol(TSRSymbolContext &context,const string symbol)
  {
   ...
   TS15NResetPool(context.delayed_decision);
   ...
   for(int n=0;n<TS15J_POOL_CAPACITY;++n)
     {
      ...
      TSRCsvAppend(line,r.existing_risk>0.0?TSRDouble(r.existing_risk):"");TSRCsvAppend(line,r.existing_tp_distance>0.0?TSRDouble(r.existing_tp_distance):"");TSRCsvAppend(line,r.atr14_m5>0.0&&r.existing_risk>0.0?TSRDouble(r.existing_risk/r.atr14_m5,12):"");TSRCsvAppend(line,r.atr14_m5>0.0&&r.existing_tp_distance>0.0?TSRDouble(r.existing_tp_distance/r.atr14_m5,12):"");TSRCsvAppend(line,TS15GRiskSourceName(r.existing_source));TSRCsvAppend(line,TS15GResultName(r.existing_continuation_result));TSRCsvAppend(line,r.existing_continuation_touch_msc>0?TSRLong(r.existing_continuation_touch_msc):"");TSRCsvAppend(line,TS15GResultName(r.existing_reversal_result));TSRCsvAppend(line,r.existing_reversal_touch_msc>0?TSRLong(r.existing_reversal_touch_msc):"");TSRCsvAppend(line,r.invalid?"INVALID_PATH":(r.censored_horizon_lag?"CENSORED_HORIZON_LAG":(r.censored?"CENSORED_END_OF_DATA":(r.feature_valid?"COMPLETE_3600S":"COMPLETE_3600S_FEATURE_UNAVAILABLE"))));TSRCsvAppend(line,TSRLong(r.quote_count));TSRCsvAppend(line,TSRLong(r.duplicate_same_msc));TSRCsvAppend(line,TSRLong(r.future_reads));TSRCsvAppend(line,TSRLong(r.backdates));TSRCsvAppend(line,TSRLong(r.fallback_quotes));
      for(int h=0;h<TS15J_HORIZONS;++h){TSRCsvAppend(line,TSRLong(r.horizon_target_msc[h]));TSRCsvAppend(line,(r.horizon_done[h]||r.horizon_censored[h])?TSRLong(r.horizon_quote_msc[h]):"");TSRCsvAppend(line,(r.horizon_done[h]||r.horizon_censored[h])?TSRLong(r.horizon_lag_ms[h]):"");TSRCsvAppend(line,r.horizon_done[h]?"AVAILABLE":(r.horizon_censored[h]?"CENSORED_HORIZON_LAG":"MISSING"));TSRCsvAppend(line,r.horizon_done[h]?TSRDouble(r.continuation_mfe_h[h]):"");TSRCsvAppend(line,r.horizon_done[h]?TSRDouble(r.continuation_mae_h[h]):"");TSRCsvAppend(line,r.horizon_done[h]?TSRDouble(r.reversal_mfe_h[h]):"");TSRCsvAppend(line,r.horizon_done[h]?TSRDouble(r.reversal_mae_h[h]):"");}
      ...
      TSRCsvAppend(line,TS15JSchema());TSMt5WriteLine(g_post_shock_excursion_file,line);++g_post_shock_excursion_rows;if(r.invalid)++pool.invalid_paths;else if(r.censored_horizon_lag)++pool.horizon_lag_censored;else if(!r.censored)++pool.completed;
      TS15JResetRecord(r);pool.records[n]=r;
     }
   TSMt5Flush(g_post_shock_excursion_file);
  }

void TSR15NWritePending(TickShock15NPool &pool)
  {
   for(int n=0;n<TS15N_POOL_CAPACITY;++n)
     {
      TickShock15NRecord r=pool.records[n];if(!r.write_pending)continue;
      for(int c=0;c<TS15N_CHECKPOINTS;++c)
        {
         TickShock15NCheckpoint s=r.checkpoint[c];string line="";TSRCsvAppend(line,r.episode_id);TSRCsvAppend(line,r.event_id);TSRCsvAppend(line,TSRLong(r.market_cluster_id));TSRCsvAppend(line,r.symbol);TSRCsvAppend(line,TSRDirection(r.shock_direction));TSRCsvAppend(line,TSRLong(r.t0_msc));TSRCsvAppend(line,IntegerToString(s.delay_seconds));TSRCsvAppend(line,TSRLong(s.target_msc));TSRCsvAppend(line,TS15NStatusName(s.status));
         TSRCsvAppend(line,s.decided?TSRLong(s.decision_quote_msc):"");TSRCsvAppend(line,s.decided?TSRLong(s.decision_processing_msc):"");TSRCsvAppend(line,s.decided?TSRLong(s.feature_max_source_msc):"");TSRCsvAppend(line,s.decided?TSRLong(s.atr_source_msc):"");TSRCsvAppend(line,s.decided?TSRLong(s.checkpoint_quote_lag_ms):"");TSRCsvAppend(line,s.decided?TSRDouble(s.decision_bid):"");TSRCsvAppend(line,s.decided?TSRDouble(s.decision_ask):"");TSRCsvAppend(line,s.decided?TSRDouble(s.decision_mid):"");TSRCsvAppend(line,TSRDouble(r.atr_t0));TSRCsvAppend(line,s.decided?TSRDouble(s.atr_decision):"");
         TSRCsvAppend(line,s.decided?TSRDouble(s.postshock_return_atr,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.postshock_mfe_atr,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.postshock_mae_atr,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.postshock_range_atr,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.retracement_from_peak_pct,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.retracement_from_trough_pct,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.current_location_in_range,12):"");TSRCsvAppend(line,s.decided?TSRLong(s.new_extreme_count):"");TSRCsvAppend(line,s.decided?TSRLong(s.origin_recross_count):"");TSRCsvAppend(line,s.decided?TSRLong(s.time_since_last_extreme_ms):"");TSRCsvAppend(line,s.decided?TSRDouble(s.distance_from_last_extreme_atr,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.net_move_over_path_length,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.shock_direction_tick_ratio,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.direction_consistency,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.realized_abs_move_atr,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.realized_volatility,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.recent_5s_acceleration,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.recent_10s_acceleration,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.peak_update_interval_ms,4):"");TSRCsvAppend(line,s.decided?TSRDouble(s.extreme_update_rate,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.path_contraction_ratio,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.path_expansion_ratio,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.decision_spread_atr,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.spread_vs_t0_ratio,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.spread_vs_postshock_mean,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.spread_contraction_from_t0,12):"");TSRCsvAppend(line,s.decided?TSRLong(s.tick_count_postshock):"");TSRCsvAppend(line,s.decided?TSRDouble(s.tick_rate_recent_5s,8):"");TSRCsvAppend(line,s.decided?TSRDouble(s.tick_rate_recent_10s,8):"");TSRCsvAppend(line,s.decided?TSRDouble(s.tick_activity_vs_t0,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.activity_decay,12):"");TSRCsvAppend(line,s.decided?TSRDouble(s.activity_acceleration,8):"");TSRCsvAppend(line,TS15NSchema());TSMt5WriteLine(g_delayed_checkpoint_file,line);++g_delayed_checkpoint_rows;
         for(int a=0;a<TS15N_ACTIONS;++a)
           {
            TickShock15NAction x=s.action[a];string row="";TSRCsvAppend(row,r.episode_id);TSRCsvAppend(row,r.event_id);TSRCsvAppend(row,TSRLong(r.market_cluster_id));TSRCsvAppend(row,r.symbol);TSRCsvAppend(row,TSRDirection(r.shock_direction));TSRCsvAppend(row,TSRLong(r.t0_msc));TSRCsvAppend(row,IntegerToString(s.delay_seconds));TSRCsvAppend(row,TS15NStatusName(s.status));TSRCsvAppend(row,TS15NActionName(a));TSRCsvAppend(row,TSRDirection(x.direction));TSRCsvAppend(row,s.decided?TSRLong(s.decision_quote_msc):"");TSRCsvAppend(row,s.decided?TSRLong(s.decision_processing_msc):"");TSRCsvAppend(row,s.decided?TSRLong(s.feature_max_source_msc):"");TSRCsvAppend(row,x.entry_eligible_msc>0?TSRLong(x.entry_eligible_msc):"");TSRCsvAppend(row,x.entered?TSRLong(x.entry_quote_msc):"");TSRCsvAppend(row,x.entered?TSRLong(x.entry_processing_msc):"");TSRCsvAppend(row,x.entered?TSRLong(x.entry_quote_msc-s.decision_quote_msc):"");TSRCsvAppend(row,x.entered?TSRDouble(x.entry_bid):"");TSRCsvAppend(row,x.entered?TSRDouble(x.entry_ask):"");TSRCsvAppend(row,x.entered?TSRDouble(x.entry_price):"");TSRCsvAppend(row,s.decided?TSRDouble(s.atr_decision):"");TSRCsvAppend(row,x.entered?TSRDouble(x.risk_distance):"");TSRCsvAppend(row,x.entered?TSRDouble(x.tp_distance):"");TSRCsvAppend(row,x.entered?TSRDouble(x.sl):"");TSRCsvAppend(row,x.entered?TSRDouble(x.tp):"");TSRCsvAppend(row,x.done?TSRLong(x.exit_msc):"");TSRCsvAppend(row,x.done?TSRDouble(x.exit_price):"");TSRCsvAppend(row,TS15NResultName(x.result));TSRCsvAppend(row,x.done?TSRDouble(x.realized_r,12):"");TSRCsvAppend(row,x.entered?TSRDouble(x.mfe_r,12):"");TSRCsvAppend(row,x.entered?TSRDouble(x.mae_r,12):"");TSRCsvAppend(row,x.done?TSRDouble(x.realized_r,12):"");TSRCsvAppend(row,x.done?TSRDouble(x.realized_r-0.02,12):"");TSRCsvAppend(row,x.done?TSRDouble(x.realized_r-0.05,12):"");TSRCsvAppend(row,x.done?TSRDouble(x.realized_r-0.10,12):"");TSRCsvAppend(row,TSRLong(r.deadline_msc));TSRCsvAppend(row,TS15NSchema());TSMt5WriteLine(g_delayed_action_file,row);++g_delayed_action_rows;
           }
        }
      ++pool.completed;TS15NResetRecord(r);pool.records[n]=r;
     }
   TSMt5Flush(g_delayed_checkpoint_file);TSMt5Flush(g_delayed_action_file);
  }

bool TSR15OArmEpisode(const int target_index,const string episode_id,const string event_id,const long cluster_id,const int direction,
                      const long t0_msc,const long t0_quote_msc,const long processing_msc)
  {
   int n=ArraySize(g_symbols);if(n!=TS15O_SYMBOLS||ArraySize(g_cross_fx_quotes)!=n||ArraySize(g_cross_fx_pools)!=n)return false;
   TickShock15OQuoteState states[];string symbols[];double atrs[];long atr_sources[];ArrayResize(states,n);ArrayResize(symbols,n);ArrayResize(atrs,n);ArrayResize(atr_sources,n);
   for(int i=0;i<n;++i){states[i]=g_cross_fx_quotes[i];symbols[i]=g_symbols[i].symbol;TS15LATRAtOffset(g_symbols[i].context_features.bars,0,atrs[i],atr_sources[i]);}
   TickShock15OSnapshot snapshot;if(!TS15OBuildSnapshot(states,symbols,atrs,atr_sources,target_index,episode_id,event_id,cluster_id,direction,t0_msc,t0_quote_msc,processing_msc,InpMaxQuoteAgeMs,snapshot))return false;
   return TS15OArm(g_cross_fx_pools[target_index],snapshot);
  }

void TSR15OWritePending(TickShock15OPool &pool)
  {
   if(g_cross_fx_feature_file==INVALID_HANDLE||g_cross_fx_freshness_file==INVALID_HANDLE||g_cross_fx_action_file==INVALID_HANDLE)return;
   for(int i=0;i<TS15O_POOL_CAPACITY;++i)
     {
      TickShock15ORecord r=pool.records[i];if(!r.write_pending)continue;TickShock15OSnapshot s=r.snapshot;
      string line="";TSRCsvAppend(line,s.episode_id);TSRCsvAppend(line,s.event_id);TSRCsvAppend(line,TSRLong(s.market_cluster_id));TSRCsvAppend(line,s.target_symbol);TSRCsvAppend(line,TSRDirection(s.shock_direction));TSRCsvAppend(line,IntegerToString(s.target_usd_sign));TSRCsvAppend(line,TSRLong(s.t0_msc));TSRCsvAppend(line,TSRLong(s.t0_quote_msc));TSRCsvAppend(line,TSRLong(s.t0_processing_msc));TSRCsvAppend(line,TSRDouble(s.atr14_m5,12));TSRCsvAppend(line,TSRLong(s.atr_source_msc));TSRCsvAppend(line,TS15OStatusName(s.status));TSRCsvAppend(line,s.status_reason);
      for(int w=0;w<TS15O_WINDOWS;++w){bool ready=s.target_return_valid[w]&&s.valid_cross_symbols[w]>0;TSRCsvAppend(line,s.target_return_valid[w]?TSRDouble(s.target_usd_return_atr[w],12):"");TSRCsvAppend(line,IntegerToString(s.valid_cross_symbols[w]));TSRCsvAppend(line,IntegerToString(s.breadth_count[w]));TSRCsvAppend(line,s.valid_cross_symbols[w]>0?TSRDouble(s.breadth[w],12):"");TSRCsvAppend(line,s.valid_cross_symbols[w]>0?TSRDouble(s.consensus_median[w],12):"");TSRCsvAppend(line,s.valid_cross_symbols[w]>0?TSRDouble(s.aligned_consensus[w],12):"");TSRCsvAppend(line,ready?TSRDouble(s.target_residual[w],12):"");TSRCsvAppend(line,ready?TSRDouble(s.aligned_residual[w],12):"");}
      TSRCsvAppend(line,TSRLong(s.median_cross_quote_age_ms));TSRCsvAppend(line,TSRLong(s.max_cross_quote_age_ms));TSRCsvAppend(line,IntegerToString(s.num_prior_cross_fx_movers));TSRCsvAppend(line,s.num_prior_cross_fx_movers>0?TSRLong(s.first_cross_fx_lead_ms):"");TSRCsvAppend(line,s.num_prior_cross_fx_movers>0?TSRLong(s.median_cross_fx_lead_ms):"");TSRCsvAppend(line,s.target_leader_rank>0?IntegerToString(s.target_leader_rank):"");TSRCsvAppend(line,s.target_lead_bucket);TSRCsvAppend(line,TSRBool(s.h1));TSRCsvAppend(line,TSRBool(s.h2));TSRCsvAppend(line,TSRBool(s.h3));TSRCsvAppend(line,TSRBool(s.h4));TSRCsvAppend(line,TSRLong(s.future_sources));TSRCsvAppend(line,TS15OFeatureSpecHash());TSRCsvAppend(line,TS15OSchema());TSMt5WriteLine(g_cross_fx_feature_file,line);++g_cross_fx_feature_rows;
      for(int source=0;source<TS15O_SYMBOLS;++source)for(int w=0;w<TS15O_WINDOWS;++w){TickShock15OSource x=s.sources[source*TS15O_WINDOWS+w];string f="";TSRCsvAppend(f,s.episode_id);TSRCsvAppend(f,s.event_id);TSRCsvAppend(f,TSRLong(s.market_cluster_id));TSRCsvAppend(f,s.target_symbol);TSRCsvAppend(f,x.symbol);TSRCsvAppend(f,IntegerToString(x.window_seconds));TSRCsvAppend(f,TSRBool(x.valid));TSRCsvAppend(f,x.reason);TSRCsvAppend(f,x.current_quote_msc>0?TSRLong(x.current_quote_msc):"");TSRCsvAppend(f,x.current_processing_msc>0?TSRLong(x.current_processing_msc):"");TSRCsvAppend(f,x.current_mid>0.0?TSRDouble(x.current_mid):"");TSRCsvAppend(f,x.anchor_quote_msc>0?TSRLong(x.anchor_quote_msc):"");TSRCsvAppend(f,x.anchor_processing_msc>0?TSRLong(x.anchor_processing_msc):"");TSRCsvAppend(f,x.anchor_mid>0.0?TSRDouble(x.anchor_mid):"");TSRCsvAppend(f,x.current_quote_msc>0?TSRLong(x.quote_age_ms):"");TSRCsvAppend(f,x.anchor_quote_msc>0?TSRLong(x.anchor_age_ms):"");TSRCsvAppend(f,x.atr14_m5>0.0?TSRDouble(x.atr14_m5,12):"");TSRCsvAppend(f,x.atr_source_msc>0?TSRLong(x.atr_source_msc):"");TSRCsvAppend(f,IntegerToString(x.usd_orientation));TSRCsvAppend(f,x.valid?TSRDouble(x.usd_return_atr,12):"");TSRCsvAppend(f,x.crossing_msc>0?TSRLong(x.crossing_msc):"");TSRCsvAppend(f,TS15OFeatureSpecHash());TSRCsvAppend(f,TS15OSchema());TSMt5WriteLine(g_cross_fx_freshness_file,f);++g_cross_fx_freshness_rows;}
      for(int a=0;a<TS15O_ACTIONS;++a){TickShock15OAction x=r.actions[a];string o="";TSRCsvAppend(o,s.episode_id);TSRCsvAppend(o,s.event_id);TSRCsvAppend(o,TSRLong(s.market_cluster_id));TSRCsvAppend(o,s.target_symbol);TSRCsvAppend(o,TSRDirection(s.shock_direction));TSRCsvAppend(o,TSRLong(s.t0_msc));TSRCsvAppend(o,TS15OStatusName(s.status));TSRCsvAppend(o,TSRBool(s.h1));TSRCsvAppend(o,TSRBool(s.h2));TSRCsvAppend(o,TSRBool(s.h3));TSRCsvAppend(o,TSRBool(s.h4));TSRCsvAppend(o,TS15OActionName(a));TSRCsvAppend(o,TSRDirection(x.direction));TSRCsvAppend(o,TSRLong(x.entry_eligible_msc));TSRCsvAppend(o,x.entered?TSRLong(x.entry_quote_msc):"");TSRCsvAppend(o,x.entered?TSRLong(x.entry_processing_msc):"");TSRCsvAppend(o,x.entered?TSRDouble(x.entry_bid):"");TSRCsvAppend(o,x.entered?TSRDouble(x.entry_ask):"");TSRCsvAppend(o,x.entered?TSRDouble(x.entry_price):"");TSRCsvAppend(o,TSRDouble(s.atr14_m5,12));TSRCsvAppend(o,x.entered?TSRDouble(x.risk_distance,12):"");TSRCsvAppend(o,x.entered?TSRDouble(x.tp_distance,12):"");TSRCsvAppend(o,x.entered?TSRDouble(x.sl):"");TSRCsvAppend(o,x.entered?TSRDouble(x.tp):"");TSRCsvAppend(o,x.done?TSRLong(x.exit_msc):"");TSRCsvAppend(o,x.done?TSRDouble(x.exit_price):"");TSRCsvAppend(o,TS15OResultName(x.result));TSRCsvAppend(o,x.done?TSRDouble(x.realized_r,12):"");TSRCsvAppend(o,x.done?TSRDouble(x.realized_r-0.02,12):"");TSRCsvAppend(o,x.done?TSRDouble(x.realized_r-0.05,12):"");TSRCsvAppend(o,x.done?TSRDouble(x.realized_r-0.10,12):"");TSRCsvAppend(o,x.entered?TSRDouble(x.mfe_r,12):"");TSRCsvAppend(o,x.entered?TSRDouble(x.mae_r,12):"");TSRCsvAppend(o,TSRLong(r.deadline_msc));TSRCsvAppend(o,TS15OFeatureSpecHash());TSRCsvAppend(o,TS15OSchema());TSMt5WriteLine(g_cross_fx_action_file,o);++g_cross_fx_action_rows;}
      ++pool.completed;if(r.invalid)++pool.invalid_paths;TS15OResetRecord(r);pool.records[i]=r;
     }
   TSMt5Flush(g_cross_fx_feature_file);TSMt5Flush(g_cross_fx_freshness_file);TSMt5Flush(g_cross_fx_action_file);
  }

void TSR15PWritePending(TickShock15PPool &pool)
  {
   if(g_symmetric_oco_file==INVALID_HANDLE)return;
   for(int n=0;n<TS15P_POOL_CAPACITY;++n)
     {
      TickShock15PRecord r=pool.records[n];if(!r.write_pending)continue;
      for(int o=0;o<TS15P_OFFSET_COUNT;++o)
        {
         ...
         for(int tp=0;tp<TS15P_TP_COUNT;++tp)for(int sl=0;sl<TS15P_SL_COUNT;++sl)
           {
            int k=TS15PScenarioIndex(tp,sl);TickShock15PScenario s=leg.scenarios[k];string line="";
            TSRCsvAppend(line,r.episode_id);TSRCsvAppend(line,r.event_id);TSRCsvAppend(line,TSRLong(r.market_cluster_id));TSRCsvAppend(line,r.symbol);TSRCsvAppend(line,TSRDirection(r.shock_direction));
            TSRCsvAppend(line,TSRLong(r.t0_msc));TSRCsvAppend(line,TSRLong(r.t0_quote_msc));TSRCsvAppend(line,TSRLong(r.t0_processing_msc));TSRCsvAppend(line,TSRDouble(r.t0_bid));TSRCsvAppend(line,TSRDouble(r.t0_ask));TSRCsvAppend(line,TSRDouble(r.t0_mid));TSRCsvAppend(line,TSRDouble(r.atr14_m5,12));TSRCsvAppend(line,TSRLong(r.atr_source_msc));TSRCsvAppend(line,TSRDouble(r.point));TSRCsvAppend(line,TSRDouble(r.pip_size));
            TSRCsvAppend(line,IntegerToString(o));TSRCsvAppend(line,TSRDouble(leg.offset_atr,6));TSRCsvAppend(line,TSRDouble(leg.upper_trigger));TSRCsvAppend(line,TSRDouble(leg.lower_trigger));TSRCsvAppend(line,TS15PEntryStatusName(leg.status));TSRCsvAppend(line,leg.direction==0?"NONE":TSRDirection(leg.direction));
            TSRCsvAppend(line,leg.trigger_msc>0?TSRLong(leg.trigger_msc):"");TSRCsvAppend(line,leg.trigger_msc>0?TSRDouble(leg.trigger_bid):"");TSRCsvAppend(line,leg.trigger_msc>0?TSRDouble(leg.trigger_ask):"");TSRCsvAppend(line,leg.entry_msc>0?TSRLong(leg.entry_msc):"");TSRCsvAppend(line,leg.entry_msc>0?TSRLong(leg.entry_processing_msc):"");TSRCsvAppend(line,leg.entry_msc>0?TSRDouble(leg.entry_bid):"");TSRCsvAppend(line,leg.entry_msc>0?TSRDouble(leg.entry_ask):"");TSRCsvAppend(line,leg.entry_msc>0?TSRDouble(leg.entry_price):"");TSRCsvAppend(line,leg.entry_msc>0?TSRDouble(leg.entry_spread):"");TSRCsvAppend(line,leg.entry_msc>0&&r.pip_size>0.0?TSRDouble(leg.entry_spread/r.pip_size,8):"");TSRCsvAppend(line,leg.deadline_msc>0?TSRLong(leg.deadline_msc):"");
            TSRCsvAppend(line,IntegerToString(tp));TSRCsvAppend(line,TSRDouble(s.tp_atr,6));TSRCsvAppend(line,IntegerToString(sl));TSRCsvAppend(line,TSRDouble(s.sl_atr,6));TSRCsvAppend(line,leg.entry_msc>0?TSRDouble(s.tp_price):"");TSRCsvAppend(line,leg.entry_msc>0?TSRDouble(s.sl_price):"");TSRCsvAppend(line,TS15PResultName(s.result));TSRCsvAppend(line,s.exit_msc>0?TSRLong(s.exit_msc):"");TSRCsvAppend(line,s.exit_msc>0?TSRDouble(s.exit_bid):"");TSRCsvAppend(line,s.exit_msc>0?TSRDouble(s.exit_ask):"");TSRCsvAppend(line,s.exit_msc>0?TSRDouble(s.exit_price):"");TSRCsvAppend(line,s.exit_msc>0?TSRDouble(s.gross_price,12):"");TSRCsvAppend(line,s.exit_msc>0?TSRDouble(s.gross_pips,8):"");TSRCsvAppend(line,s.exit_msc>0?TSRDouble(s.gross_r,12):"");TSRCsvAppend(line,leg.entry_msc>0?TSRDouble(s.mfe_price,12):"");TSRCsvAppend(line,leg.entry_msc>0?TSRDouble(s.mae_price,12):"");
            TSRCsvAppend(line,TSRLong(r.quote_groups));TSRCsvAppend(line,TSRLong(r.same_msc_ticks));TSRCsvAppend(line,TSRLong(r.future_reads));TSRCsvAppend(line,TSRLong(r.backdates));TSRCsvAppend(line,TSRLong(r.fallback_quotes));TSRCsvAppend(line,TSRLong(r.ambiguous_triggers));TSRCsvAppend(line,TSRBool(r.invalid));TSRCsvAppend(line,TSRBool(r.censored));TSRCsvAppend(line,TS15PSchema());
            TSMt5WriteLine(g_symmetric_oco_file,line);++g_symmetric_oco_rows;
           }
        }
      ++pool.completed;if(r.invalid)++pool.invalid_paths;pool.ambiguous_triggers+=r.ambiguous_triggers;TS15PResetRecord(r);pool.records[n]=r;
     }
   TSMt5Flush(g_symmetric_oco_file);
  }
      ...
      if(TS15EArmEpisode(context.medium_horizon,InpRunId,context.symbol,candidate.statistical_event_id,candidate.statistical_market_cluster_id,candidate.direction,confirmed_msc,processing_msc,confirmed_point.bid,confirmed_point.ask,context.point,context.tick_size,MathAbs(confirmed_point.mid-candidate.anchor_mid),candidate.score[candidate.trigger_horizon_index],false,InpSubmitLatencyMs))
        {
         ...
         TickShock15LSnapshot clean_features;TS15LBuildSnapshot(context.clean_move_features,context.context_features.bars,episode.episode_id,candidate.statistical_event_id,context.symbol,candidate.statistical_market_cluster_id,candidate.direction,t0.t0_msc,t0.atr14_m5,clean_features);TSR15LWriteSnapshot(clean_features);
          ...
          TS15NArm(context.delayed_decision,episode.episode_id,candidate.statistical_event_id,context.symbol,candidate.statistical_market_cluster_id,candidate.direction,t0.t0_msc,t0.confirmed_quote_msc,processing_msc,confirmed_point.bid,confirmed_point.ask,t0.atr14_m5,context.tick_size,candidate.tick_intensity_ratio);
          if(!TSR15OArmEpisode(symbol_index,episode.episode_id,candidate.statistical_event_id,candidate.statistical_market_cluster_id,candidate.direction,t0.t0_msc,t0.confirmed_quote_msc,processing_msc))
             g_event_engine.validation_invalid=true;
          double oco_atr=0.0;long oco_atr_source=0;TS15LATRAtOffset(context.context_features.bars,0,oco_atr,oco_atr_source);
          if(oco_atr>0.0&&oco_atr_source>0&&!TS15PArm(context.symmetric_oco,episode.episode_id,candidate.statistical_event_id,context.symbol,candidate.statistical_market_cluster_id,candidate.direction,t0.t0_msc,t0.confirmed_quote_msc,processing_msc,confirmed_point.bid,confirmed_point.ask,oco_atr,oco_atr_source,context.digits,context.point))
             g_event_engine.validation_invalid=true;
        }
     }
   else
...
void TSRProcessOneTick(const int symbol_index,const MqlTick &source,const long processing_msc,const bool close_equal_boundary)
  {
   ...
   TS15OObserveQuote(g_cross_fx_quotes[symbol_index],time_msc,processing_msc,tick.bid,tick.ask);
   ...
   double decision_atr=0.0;long decision_atr_source=0;TS15LATRAtOffset(g_symbols[symbol_index].context_features.bars,0,decision_atr,decision_atr_source);
   TS15NObservePool(g_symbols[symbol_index].delayed_decision,time_msc,processing_msc,tick.bid,tick.ask,decision_atr,decision_atr_source,false,InpMaxQuoteAgeMs,InpSubmitLatencyMs);
   TSR15NWritePending(g_symbols[symbol_index].delayed_decision);
   TS15OObservePool(g_cross_fx_pools[symbol_index],time_msc,processing_msc,tick.bid,tick.ask,false);
...
void TSRWriteSummary()
  {
   ...
   long excursion_armed=0,excursion_completed=0,excursion_censored=0,excursion_lag_censored=0,excursion_capacity=0,excursion_invalid=0;for(int i=0;i<ArraySize(g_symbols);++i){excursion_armed+=g_symbols[i].post_shock_excursion.armed;excursion_completed+=g_symbols[i].post_shock_excursion.completed;excursion_censored+=g_symbols[i].post_shock_excursion.censored;excursion_lag_censored+=g_symbols[i].post_shock_excursion.horizon_lag_censored;excursion_capacity+=g_symbols[i].post_shock_excursion.capacity_hits;excursion_invalid+=g_symbols[i].post_shock_excursion.invalid_paths;}
   TSRSummaryRow("POST_SHOCK_EXCURSION","episodes",excursion_armed,0,0,excursion_completed,excursion_invalid+excursion_censored+excursion_lag_censored,0.0,StringFormat("rows=%I64d;completed_3600s=%I64d;censored=%I64d;horizon_lag_censored=%I64d;max_horizon_lag_ms=%d;invalid_paths=%I64d;capacity_hits=%I64d;pool_per_symbol=%d;orders=0",g_post_shock_excursion_rows,excursion_completed,excursion_censored,excursion_lag_censored,TS15K_MAX_HORIZON_LAG_MS,excursion_invalid,excursion_capacity,TS15J_POOL_CAPACITY));
   ...
   long delayed_armed=0,delayed_completed=0,delayed_capacity=0;for(int i=0;i<ArraySize(g_symbols);++i){delayed_armed+=g_symbols[i].delayed_decision.armed;delayed_completed+=g_symbols[i].delayed_decision.completed;delayed_capacity+=g_symbols[i].delayed_decision.capacity_hits;}
   TSRSummaryRow("LOG_POLICY","delayed_decision_csv",0,0,0,delayed_completed,delayed_capacity,0.0,StringFormat("episodes=%I64d;checkpoint_rows=%I64d;action_rows=%I64d;checkpoints=15,30,60,120;deadline=t0_plus_900s;atr=latest_completed_M5_ATR14;second_ring_capacity=%d;tick_csv=false;orders=0",delayed_armed,g_delayed_checkpoint_rows,g_delayed_action_rows,TS15N_SECOND_CAPACITY));
   long cross_armed=0,cross_completed=0,cross_capacity=0,cross_invalid=0;for(int i=0;i<ArraySize(g_cross_fx_pools);++i){cross_armed+=g_cross_fx_pools[i].armed;cross_completed+=g_cross_fx_pools[i].completed;cross_capacity+=g_cross_fx_pools[i].capacity_hits;cross_invalid+=g_cross_fx_pools[i].invalid_paths;}
   TSRSummaryRow("LOG_POLICY","cross_fx_lead_lag_csv",0,0,0,cross_completed,cross_capacity,0.0,StringFormat("episodes=%I64d;feature_rows=%I64d;freshness_rows=%I64d;action_rows=%I64d;invalid_paths=%I64d;windows=1,3,5,10;lead_threshold=0.10ATR_5s;quote_ring_per_symbol=%d;tick_csv=false;orders=0",cross_armed,g_cross_fx_feature_rows,g_cross_fx_freshness_rows,g_cross_fx_action_rows,cross_invalid,TS15O_QUOTE_CAPACITY));
   long oco_armed=0,oco_completed=0,oco_capacity=0,oco_invalid=0,oco_ambiguous=0;for(int i=0;i<ArraySize(g_symbols);++i){oco_armed+=g_symbols[i].symmetric_oco.armed;oco_completed+=g_symbols[i].symmetric_oco.completed;oco_capacity+=g_symbols[i].symmetric_oco.capacity_hits;oco_invalid+=g_symbols[i].symmetric_oco.invalid_paths;oco_ambiguous+=g_symbols[i].symmetric_oco.ambiguous_triggers;}
   TSRSummaryRow("LOG_POLICY","symmetric_oco_scenarios_csv",0,0,0,oco_completed,oco_capacity,0.0,StringFormat("episodes=%I64d;scenario_rows=%I64d;invalid_paths=%I64d;ambiguous_triggers=%I64d;offsets=0.02,0.05,0.10ATR;tp=0.02,0.03,0.05,0.075,0.10,0.15ATR;sl=0.10,0.20,0.30,0.50,0.75,1.00ATR;trigger_wait=30s;hold_from_entry=300s;tick_csv=false;orders=0",oco_armed,g_symmetric_oco_rows,oco_invalid,oco_ambiguous));
...
int OnInit()
  {
   ...
   if(ArraySize(g_symbols)!=TS15O_SYMBOLS)
     {PrintFormat("%s Step15O requires exactly %d symbols",TSR_NAME,TS15O_SYMBOLS);return INIT_PARAMETERS_INCORRECT;}
   ArrayResize(g_cross_fx_quotes,ArraySize(g_symbols));ArrayResize(g_cross_fx_pools,ArraySize(g_symbols));
   for(int i=0;i<ArraySize(g_symbols);++i){TS15OResetQuoteState(g_cross_fx_quotes[i]);TS15OResetPool(g_cross_fx_pools[i]);}
   for(int i=0;i<ArraySize(g_symbols);++i){TS15GResetContext(g_symbols[i].economic_episode);TS15GResetContext(g_symbols[i].economic_control);TS15HReset(g_symbols[i].detection_time_continuation);TS15JResetPool(g_symbols[i].post_shock_excursion);TS15NResetPool(g_symbols[i].delayed_decision);TS15PResetPool(g_symbols[i].symmetric_oco);}
   if(!TSROpenLogs())
     {
   ...
   for(int i=0;i<ArraySize(g_symbols);++i)
     {
      TS15EFinalizeEndOfData(g_symbols[i].medium_horizon,InpSubmitLatencyMs);TSR15FCaptureEpisodeFeatures(g_symbols[i]);TSR15GArmEpisodeDecisions(g_symbols[i]);TSR15EWritePending(g_symbols[i]);
       ...
       TS15NFinalizePool(g_symbols[i].delayed_decision,InpMaxQuoteAgeMs,InpSubmitLatencyMs);TSR15NWritePending(g_symbols[i].delayed_decision);
       TS15OFinalizePool(g_cross_fx_pools[i]);TSR15OWritePending(g_cross_fx_pools[i]);
       TS15PFinalizePool(g_symbols[i].symmetric_oco);TSR15PWritePending(g_symbols[i].symmetric_oco);
