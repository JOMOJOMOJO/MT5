//+------------------------------------------------------------------+
//| USDJPY EMA Continuation Long                                     |
//+------------------------------------------------------------------+
#property copyright   "Trading System"
#property link        "https://www.mql5.com"
#property version     "1.00"
#property strict
#property description "USDJPY M15 long-only continuation family using EMA13/EMA100 regime, bearish dip morphology, and explicit risk controls."
...
input double          InpMaxAdx                   = 30.0;
input int             InpSessionStartHour         = 7;
input int             InpSessionEndHour           = 16;
input double          InpMaxEma13DistancePips     = 22.0;
input double          InpMaxRet1                  = 0.0004;
input double          InpMinUpperWickShare        = 0.45;
input double          InpMaxLowerWickShare        = 0.10;
input double          InpMaxCloseLocation         = 0.45;
...
bool EvaluateSignal(const MqlRates &rates[], const double &fastEma[], const double &slowEma[], const double &adxValues[])
  {
   double pip = GetPipSize();
   if(pip <= 0.0)
      return false;
   ...
   if(adxValues[1] <= 0.0 || adxValues[1] > InpMaxAdx)
      return false;
   ...
   if(LowerWickShare(rates[1]) > InpMaxLowerWickShare)
      return false;
   if(CloseLocation(rates[1]) > InpMaxCloseLocation)
      return false;
...
void OpenPosition()
  {
   ...
   if(trade.Buy(volume, runtimeSymbol, 0.0, sl, tp, "ema_continuation_long"))
      dailyTradeCount++;
  }
...
int OnInit()
  {
   ...
   if(
      ...
      InpAdxPeriod <= 1 || InpMaxAdx <= 0.0 || InpSessionStartHour < 0 || InpSessionStartHour > 23 ||
      InpSessionEndHour < 0 || InpSessionEndHour > 23 || InpSessionStartHour == InpSessionEndHour ||
      InpMaxEma13DistancePips <= 0.0 || InpMaxRet1 < -0.01 || InpMaxRet1 > 0.01 ||
      InpMinUpperWickShare < 0.0 || InpMinUpperWickShare > 1.0 || InpMaxLowerWickShare < 0.0 || InpMaxLowerWickShare > 1.0 ||
      InpMaxCloseLocation < 0.0 || InpMaxCloseLocation > 1.0 || InpStopLossPips <= 0.0 || InpTargetRMultiple <= 0.0 ||
      InpMaxHoldBars < 1 || InpRiskPercent <= 0.0 || InpMicroCapBalanceThreshold < 0.0 || InpMicroCapRiskPercent <= 0.0 ||
...
void OnTick()
  {
   ...
   double slowEma[];
   double adxValues[];
   if(!LoadSignalWindow(rates, fastEma, slowEma, adxValues, 220))
      return;

   if(EvaluateSignal(rates, fastEma, slowEma, adxValues))
      OpenPosition();
  }
