//+------------------------------------------------------------------+
//| USDJPY Round Continuation Short Guarded                          |
//+------------------------------------------------------------------+
#property copyright   "Trading System"
#property link        "https://www.mql5.com"
#property version     "1.00"
#property strict
#property description "USDJPY M15 short-only continuation prototype using EMA13/EMA100 trend, 50-pip anti-chop state, and structure-aware stops."
...
input double          InpMaxEma13DistancePips     = 10.0;
input double          InpMinLowerWickShare        = 0.55;
input double          InpMaxUpperWickShare        = 0.12;
input double          InpMinCloseLocation         = 0.72;
input double          InpMinBodyPips              = 2.0;
input double          InpMinBodyToRange           = 0.25;
input double          InpMinSlowSlopePips         = 1.0;
input bool            InpRequireBullishSignalBar  = true;
input bool            InpRequireFastEmaTouch      = true;
input double          InpFastEmaTouchBufferPips   = 1.0;
input double          InpMaxCloseAboveFastEmaPips = 4.0;
input double          InpStopBufferPips           = 2.0;
input double          InpMinStopLossPips          = 16.0;
input double          InpMaxStopLossPips          = 30.0;
input double          InpTargetRMultiple          = 1.4;
input int             InpMaxHoldBars              = 18;
input double          InpRiskPercent              = 0.40;
input bool            InpUseMicroCapRiskOverride  = true;
input double          InpMicroCapBalanceThreshold = 150.0;
input double          InpMicroCapRiskPercent      = 1.50;
...
input double          InpDailyLossCapPercent      = 4.0;
...
input long            InpMagicNumber              = 20260610;
...
bool CanOpenAnotherTrade(datetime barTime)
  {
   ...
   if(InpUseEquityDrawdownCap && equityPeak > 0.0)
     {
      double equity = AccountInfoDouble(ACCOUNT_EQUITY);
      if(equity <= equityPeak * (1.0 - InpEquityDrawdownCapPercent / 100.0))
         return false;
     }
...
bool EvaluateSignal(const MqlRates &rates[], const double &fastEma[], const double &slowEma[])
  {
   ...
   if(slowSlopePips > -InpMinSlowSlopePips)
      return false;
   if(signalBar.high >= latestHigh.price)
      return false;

   double emaDistancePips = MathAbs(signalBar.close - fastEma[1]) / pip;
   if(emaDistancePips > InpMaxEma13DistancePips)
      return false;
   if(LowerWickShare(signalBar) < InpMinLowerWickShare)
      return false;
   if(UpperWickShare(signalBar) > InpMaxUpperWickShare)
      return false;
   if(CloseLocation(signalBar) < InpMinCloseLocation)
      return false;

   double bodyPips = BodyPips(signalBar);
   double rangePips = RangePips(signalBar);
   if(bodyPips < InpMinBodyPips || rangePips <= 0.0)
      return false;
   if((bodyPips / rangePips) < InpMinBodyToRange)
      return false;
   if(InpRequireBullishSignalBar && signalBar.close <= signalBar.open)
      return false;
   if(InpRequireFastEmaTouch && signalBar.high < fastEma[1] - (InpFastEmaTouchBufferPips * pip))
      return false;
   if(((signalBar.close - fastEma[1]) / pip) > InpMaxCloseAboveFastEmaPips)
      return false;

   plannedSignalHigh = signalBar.high;
   return true;
  }
...
void OpenPosition()
  {
   ...
   if(bid <= 0.0 || ask <= 0.0 || pip <= 0.0 || point <= 0.0 || plannedSignalHigh <= 0.0)
      return;

   double stopFloor = InpMinStopLossPips * pip;
   double stopCeiling = InpMaxStopLossPips * pip;
   double stopPrice = plannedSignalHigh + (InpStopBufferPips * pip);
   double stopDistance = stopPrice - bid;
   if(stopDistance < stopFloor)
     {
      stopDistance = stopFloor;
      stopPrice = bid + stopDistance;
     }
   if(stopDistance <= 0.0 || stopDistance > stopCeiling)
      return;
   ...
   double tp = NormalizePrice(bid - targetDistance);
   if(trade.Sell(volume, runtimeSymbol, 0.0, sl, tp, "round_continuation_short_guarded"))
      dailyTradeCount++;
  }
...
int OnInit()
  {
   ...
   if(
      ...
      InpPivotSpan < 1 || InpTrendScanBars <= 20 || InpVolatilityLookbackBars <= 5 || InpMinWindowRangePips <= 0.0 ||
      InpRoundStepPips <= 0 || InpSessionStartHour < 0 || InpSessionStartHour > 23 ||
      InpSessionEndHour < 0 || InpSessionEndHour > 24 || InpMaxEma13DistancePips <= 0.0 ||
      InpMinLowerWickShare < 0.0 || InpMinLowerWickShare > 1.0 || InpMaxUpperWickShare < 0.0 || InpMaxUpperWickShare > 1.0 ||
      InpMinCloseLocation < 0.0 || InpMinCloseLocation > 1.0 || InpMinBodyPips <= 0.0 || InpMinBodyToRange <= 0.0 || InpMinBodyToRange > 1.0 ||
      InpMinSlowSlopePips < 0.0 || InpFastEmaTouchBufferPips < 0.0 || InpMaxCloseAboveFastEmaPips < 0.0 ||
      InpStopBufferPips < 0.0 || InpMinStopLossPips <= 0.0 || InpMaxStopLossPips < InpMinStopLossPips ||
      InpTargetRMultiple <= 0.0 || InpMaxHoldBars < 1 || InpRiskPercent <= 0.0 ||
