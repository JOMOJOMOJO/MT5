//+------------------------------------------------------------------+
//| USDJPY Follow-Through Long Prototype                             |
//+------------------------------------------------------------------+
#property copyright   "Trading System"
#property link        "https://www.mql5.com"
#property version     "1.00"
#property strict
#property description "USDJPY follow-through long prototype using EMA13/EMA100, Dow-style swing trend, and volatility-aware pullback entries."
...
input double          InpTargetRMultiple             = 0.8;
input int             InpMaxHoldBars                 = 72;
...
input double          InpMaxSpreadPips               = 2.3;
...
input double          InpMinImpulsePips              = 14.0;
input double          InpMinPullbackPips             = 0.5;
input double          InpMaxPullbackPips             = 20.0;
input int             InpMaxCounterTrendBodies       = 1;
input double          InpMinTransitionDistancePips   = 0.0;
input double          InpMaxPullbackToImpulseRatio   = 0.45;
input double          InpMinRejectionBodyPips        = 0.5;
input double          InpMinRejectionCloseLocation   = 0.60;
...
input long            InpMagicNumber                 = 20260493;
...
int OnInit()
  {
   ...
   if(InpFastEMAPeriod <= 0 || InpSlowEMAPeriod <= InpFastEMAPeriod || InpPivotSpan <= 0 ||
      InpTrendScanBars <= 20 || InpSlowSlopeLookback <= 0 || InpTouchTolerancePips < 0.0 || InpMaxHoldBars < 1 ||
     ...
     {
      Print("Invalid USDJPY Follow-Through Long parameters.");
...
void OnTick()
  {
   ...
   if(CountManagedPositions() > 0)
     {
      bool closedOnBar = ManageOpenPosition(barTime);
      if(closedOnBar || CountManagedPositions() > 0)
         return;
     }
...
bool ManageOpenPosition(datetime currentBarTime)
  {
   if(InpMaxHoldBars <= 0)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(trade.PositionClose(ticket))
        {
         PrintFormat("Time exit closed position %I64u after %d bars at %s.", ticket, barsOpen, TimeToString(currentBarTime, TIME_DATE | TIME_MINUTES));
         return true;
        }

      PrintFormat("Time exit failed for position %I64u after %d bars. Error=%d", ticket, barsOpen, GetLastError());
      return false;
     }
