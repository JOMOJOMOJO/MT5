//+------------------------------------------------------------------+
//| USDJPY Late Overextension Short Guarded                          |
//+------------------------------------------------------------------+
#property copyright   "Trading System"
#property link        "https://www.mql5.com"
#property version     "1.00"
#property strict
#property description "USDJPY M15 short-only late-session overextension mean-reversion with guarded risk controls."
...
input double          InpShortMinCloseVsEma20Atr        = 1.20;
input double          InpShortMaxCloseVsEma20Atr        = 3.20;
input bool            InpUseCloseVsEma50Filter          = true;
input double          InpShortMinCloseVsEma50Atr        = 1.20;
input bool            InpUseBullTrendFilter             = true;
input bool            InpUseStackedBullTrendFilter      = false;
input bool            InpUseEma20SlopeFilter            = true;
input double          InpShortMinEma20Slope3Atr         = 0.30;
input bool            InpUseRsi7Filter                  = true;
input double          InpShortMinRsi7                   = 64.0;
input double          InpShortMaxRsi7                   = 92.0;
input bool            InpUseRsi14Filter                 = true;
input double          InpShortMinRsi14                  = 60.0;
input double          InpShortMaxRsi14                  = 86.0;
input bool            InpUseMacdLineAtrFilter           = false;
input double          InpShortMinMacdLineAtr            = 0.40;
input bool            InpUseStochFilter                 = true;
input double          InpShortMinStochK                 = 80.0;
input double          InpShortMinStochD                 = 80.0;
...
input double          InpExitBufferATR                  = 0.20;
input int             InpStructureLookbackBars          = 8;
input double          InpMinStopATR                     = 2.20;
input double          InpStructureBufferATR             = 0.25;
...
void OpenShortPosition(const SignalContext &ctx)
  {
   ...
   if(trade.Sell(volume, runtimeSymbol, 0.0, sl, tp, "late_overext_short"))
      dailyTradeCount++;
   else
...
int OnInit()
  {
   ...
   if(InpMagicNumber <= 0 || InpFastEMAPeriod <= 0 || InpSlowEMAPeriod <= 0 || InpTrendEMAPeriod <= 0 ||
     ...
     {
      Print("Invalid late-overextension parameters.");
