//+------------------------------------------------------------------+
//| USDJPY Dow Fractal Liquidity Sweep Failed Acceptance Engine      |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "Standalone liquidity-sweep failed-acceptance reversal engine."
...
enum PatternState
  {
   ...
   PATTERN_SWEEP_UP = 1,
   PATTERN_FAILED_ACCEPTANCE_ABOVE = 2,
   PATTERN_RECLAIMED_BACK_INSIDE_SHORT = 3,
   PATTERN_LOWER_HIGH_FORMED = 4,
   PATTERN_BREAKDOWN_READY = 5,
   PATTERN_SWEEP_DOWN = 6,
   PATTERN_FAILED_ACCEPTANCE_BELOW = 7,
   PATTERN_RECLAIMED_BACK_INSIDE_LONG = 8,
   PATTERN_HIGHER_LOW_FORMED = 9,
   PATTERN_BREAKOUT_READY = 10
  };
...
input string          InpTelemetryFileName        = "mt5_company_usdjpy_20260416_liquidity_sweep_failed_acceptance.csv";
input long            InpMagicNumber              = 202604161;
   ...
   switch(state)
     {
      ...
      case PATTERN_LOWER_HIGH_FORMED: return "lower_high_formed";
      ...
      case PATTERN_HIGHER_LOW_FORMED: return "higher_low_formed";
...
void FillShortPatternState(const PatternStructureContext &structure,
                           const PivotPoint &referenceHigh,
                           const PivotPoint &reclaimLow,
                           string &stateLabel)
  {
   stateLabel = PatternStateLabel(PATTERN_LOWER_HIGH_FORMED);
   if(structure.currentClose <= reclaimLow.price - structure.breakBuffer)
      stateLabel = PatternStateLabel(PATTERN_BREAKDOWN_READY);
   else if(structure.currentClose <= referenceHigh.price - structure.reclaimBuffer)
...
bool FillShortSweepFailureSetup(const ContextPhaseContext &ctx,
                                ...
                                SweepFailureSetup &setup)
  {
   ...
   double sweepSize = sweepHigh.price - referenceHigh.price;
   if(sweepSize < structure.sweepFloor || sweepSize < tierSweepFloor)
      return false;
   ...
   if(lowerHigh.price >= sweepHigh.price - structure.failureGap)
      return false;
   if(structure.currentClose > referenceHigh.price - structure.reclaimBuffer)
      return false;
   ...
   setup.patternLabel = "liquidity_sweep_failed_acceptance";
   FillShortPatternState(structure, referenceHigh, reclaimLow, setup.patternStateLabel);
   setup.volatilityBucket = structure.volatilityBucket;
   setup.referencePivot = referenceHigh;
   setup.sweepPivot = sweepHigh;
   setup.continuationPivot = reclaimLow;
   setup.failurePivot = lowerHigh;
   setup.referenceLevel = referenceHigh.price;
   setup.reclaimLevel = referenceHigh.price - structure.reclaimBuffer;
   setup.retestLevel = referenceHigh.price + structure.retestTolerance;
   setup.continuationLevel = reclaimLow.price;
   setup.stopAnchor = MathMax(sweepHigh.price, lowerHigh.price);
   setup.invalidationLevel = referenceHigh.price + PipsToPrice(InpAcceptanceExitBufferPips);
...
bool FillLongSweepFailureSetup(const ContextPhaseContext &ctx,
                               const PatternStructureContext &structure,
                               const PivotPoint &referenceLow,
                               const PivotPoint &sweepLow,
                               const PivotPoint &reclaimHigh,
                               const PivotPoint &higherLow,
                               const string tier,
                               SweepFailureSetup &setup)
  {
   ResetSweepFailureSetup(setup);
   ...
   double tierSweepFloor = (tier == "tier_a") ? PipsToPrice(InpTierAMinSweepPips) : PipsToPrice(InpTierBMinSweepPips);
   double sweepSize = referenceLow.price - sweepLow.price;
   if(sweepSize < structure.sweepFloor || sweepSize < tierSweepFloor)
      return false;
   ...
   if(higherLow.price <= sweepLow.price + structure.failureGap)
      return false;
   if(structure.currentClose < referenceLow.price + structure.reclaimBuffer)
      return false;
   ...
   setup.waveLabel = ctx.waveLabel;
   setup.patternLabel = "liquidity_sweep_failed_acceptance";
   FillLongPatternState(structure, referenceLow, reclaimHigh, setup.patternStateLabel);
   setup.volatilityBucket = structure.volatilityBucket;
   setup.referencePivot = referenceLow;
   setup.sweepPivot = sweepLow;
   setup.continuationPivot = reclaimHigh;
   setup.failurePivot = higherLow;
   setup.referenceLevel = referenceLow.price;
   setup.reclaimLevel = referenceLow.price + structure.reclaimBuffer;
   setup.retestLevel = referenceLow.price - structure.retestTolerance;
   setup.continuationLevel = reclaimHigh.price;
   setup.stopAnchor = MathMin(sweepLow.price, higherLow.price);
   setup.invalidationLevel = referenceLow.price - PipsToPrice(InpAcceptanceExitBufferPips);
...
bool DetectShortSweepFailureSetup(const ContextPhaseContext &ctx,
                                  const PatternStructureContext &structure,
   ...
   for(int i = count - 1; i >= 4; --i)
     {
      ...
      if(!(SpacingOk(p0, p1) && SpacingOk(p1, p2) && SpacingOk(p2, p3) && SpacingOk(p3, p4)))
         continue;
      if(!(p0.time < p2.time && p2.time < p3.time && p3.time < p4.time))
         continue;

      if(FillShortSweepFailureSetup(ctx, structure, p0, p2, p3, p4, tier, setup))
         return true;
     }
...
bool DetectLongSweepFailureSetup(const ContextPhaseContext &ctx,
                                 const PatternStructureContext &structure,
                                 const PivotPoint &pivots[],
                                 SweepFailureSetup &setup)
  {
   ...
   for(int i = count - 1; i >= 4; --i)
     {
      ...
      if(FillLongSweepFailureSetup(ctx, structure, p0, p2, p3, p4, tier, setup))
         return true;
     }
...
bool BuildEntryPlan(const SweepFailureSetup &setup,
                    ...
                    EntryPlan &plan)
  {
   ...
   plan.reason = "sweep_failure_" + setup.tier + "_" + setup.sideLabel + "_" + trigger.triggerLabel;
   return true;
  }
...
void ProcessNewPatternBar()
  {
   ...
   SweepFailureSetup selected;
   DetectShortSweepFailureSetup(ctx, structure, pivots, shortSetup);
   DetectLongSweepFailureSetup(ctx, structure, pivots, longSetup);
