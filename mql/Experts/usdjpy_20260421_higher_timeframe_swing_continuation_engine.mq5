...
#property description "Standalone higher-timeframe swing continuation engine."
...
enum PatternState
  {
   ...
   PATTERN_PULLBACK_IN_PROGRESS_BULL = 1,
   PATTERN_FIB_ZONE_REACHED_BULL = 2,
   PATTERN_HIGHER_LOW_CANDIDATE = 3,
   PATTERN_BULLISH_RECLAIM_READY = 4,
   PATTERN_CONTINUATION_BREAK_READY_BULL = 5,
   PATTERN_PULLBACK_IN_PROGRESS_BEAR = 6,
   PATTERN_FIB_ZONE_REACHED_BEAR = 7,
   PATTERN_LOWER_HIGH_CANDIDATE = 8,
   PATTERN_BEARISH_REJECT_READY = 9,
   PATTERN_CONTINUATION_BREAK_READY_BEAR = 10
  };

enum ExecutionTriggerMode
  {
   EXEC_RECLAIM_CLOSE_CONFIRM = 0,
   EXEC_RETEST_HOLD_OR_REJECT = 1,
...
struct ContinuationSetup
  {
   ...
   string    contextBucket;
   string    waveLabel;
   string    pullbackTypeLabel;
   string    pullbackDepthBucket;
   string    patternLabel;
...
input double          InpContextImpulseZoneMin    = 0.60;
input double          InpContextImpulseZoneMax    = 0.40;
input double          InpPullbackATRMultiple      = 0.18;
input double          InpTierAMinPullbackPips     = 6.0;
input double          InpTierBMinPullbackPips     = 3.0;
input double          InpMinContinuationPips      = 8.0;
input double          InpMinContinuationATR       = 0.35;
...
input double          InpPivotHoldToleranceATR    = 0.18;
input double          InpMinPivotHoldGapPips      = 1.0;
input double          InpTierAMinCloseLocation    = 0.55;
...
input TradeBiasMode   InpTradeBiasMode            = TRADE_BIAS_LONG_ONLY;
...
input int             InpMaxManagedPositions      = 1;
input int             InpReentryCooldownBars      = 0;
input bool            InpAllowSameDirectionReentry = false;
input double          InpMaxTotalRiskPercent      = 1.0;
input int             InpSessionStartHour         = 0;
input int             InpSessionEndHour           = 0;
input double          InpMaxSpreadPips            = 2.0;
input bool            InpEnableTelemetry          = true;
input string          InpTelemetryFileName        = "mt5_company_usdjpy_20260421_higher_timeframe_swing_continuation.csv";
...
datetime lastLongEntryTime = 0;
datetime lastShortEntryTime = 0;

long stageContextValidCount = 0;
long stageTierALongEligibleCount = 0;
long stageTierBLongEligibleCount = 0;
long stageTierAShortEligibleCount = 0;
long stageTierBShortEligibleCount = 0;
long stageFibFilterPassCount = 0;
long stagePullbackStructureBuiltCount = 0;
long stageHigherLowFormedCount = 0;
long stageLowerHighFormedCount = 0;
long stageReclaimConfirmedCount = 0;
long stageSetupValidCount = 0;
long stageTriggerFiredCount = 0;
long stageOrderSentCount = 0;

void ResetDiagnosticStageCounts()
  {
   stageContextValidCount = 0;
   stageTierALongEligibleCount = 0;
   stageTierBLongEligibleCount = 0;
   stageTierAShortEligibleCount = 0;
   stageTierBShortEligibleCount = 0;
   stageFibFilterPassCount = 0;
   stagePullbackStructureBuiltCount = 0;
   stageHigherLowFormedCount = 0;
   stageLowerHighFormedCount = 0;
   stageReclaimConfirmedCount = 0;
   stageSetupValidCount = 0;
   stageTriggerFiredCount = 0;
   stageOrderSentCount = 0;
  }
   ...
   switch(phase)
     {
      ...
      case CTX_UP_PULLBACK: return "ctx_up_pullback";
      case CTX_UP_EXHAUSTION: return "ctx_up_exhaustion";
      case CTX_RANGE: return "ctx_range";
      case CTX_DOWN_IMPULSE: return "ctx_down_impulse";
      case CTX_DOWN_PULLBACK: return "ctx_down_pullback";
...
string PatternStateLabel(PatternState state)
  {
   switch(state)
     {
      case PATTERN_PULLBACK_IN_PROGRESS_BULL:
      case PATTERN_PULLBACK_IN_PROGRESS_BEAR: return "pullback_in_progress";
      case PATTERN_FIB_ZONE_REACHED_BULL:
      case PATTERN_FIB_ZONE_REACHED_BEAR: return "fib_zone_reached";
      case PATTERN_HIGHER_LOW_CANDIDATE: return "higher_low_candidate";
      case PATTERN_LOWER_HIGH_CANDIDATE: return "lower_high_candidate";
      case PATTERN_BULLISH_RECLAIM_READY: return "bullish_reclaim_ready";
      case PATTERN_BEARISH_REJECT_READY: return "bearish_reject_ready";
      case PATTERN_CONTINUATION_BREAK_READY_BULL:
      case PATTERN_CONTINUATION_BREAK_READY_BEAR: return "continuation_break_ready";
      default: return "none";
     }
  }
...
string ExecutionTriggerModeLabel(ExecutionTriggerMode mode)
  {
   switch(mode)
     {
      case EXEC_RETEST_HOLD_OR_REJECT: return "exec_retest_hold_or_reject";
...
void ResetContinuationSetup(ContinuationSetup &setup)
  {
   setup.valid = false;
   setup.direction = 0;
   setup.tier = "";
   ...
   setup.contextBucket = "";
   setup.waveLabel = "";
   setup.pullbackTypeLabel = "";
   setup.pullbackDepthBucket = "";
   setup.patternLabel = "";
...
void ResetEntryPlan(EntryPlan &plan)
  {
   ...
   plan.contextBucket = "";
   plan.waveLabel = "";
   plan.pullbackTypeLabel = "";
   plan.pullbackDepthBucket = "";
   plan.patternLabel = "";
...
void ResetPendingExecution(PendingExecutionContext &ctx)
  {
   ctx.valid = false;
   ResetContinuationSetup(ctx.setup);
...
ContextPhase ClassifyContextPhase(const string trendLabel,
                                  ...
                                  double retraceRatio)
  {
   if(trendLabel == "hh_hl")
     {
      if(closePrice >= emaFast && rangePosition >= InpContextImpulseZoneMin && retraceRatio <= 0.382)
         return CTX_UP_IMPULSE;
      if(closePrice >= emaSlow && retraceRatio >= 0.236 && retraceRatio <= 0.786)
         return CTX_UP_PULLBACK;
      if(rangePosition >= InpContextImpulseZoneMin && (closePrice < emaFast || retraceRatio > 0.382))
   ...
   if(trendLabel == "lh_ll")
     {
      if(closePrice <= emaFast && rangePosition <= InpContextImpulseZoneMax && retraceRatio <= 0.382)
         return CTX_DOWN_IMPULSE;
      if(closePrice <= emaSlow && retraceRatio >= 0.236 && retraceRatio <= 0.786)
         return CTX_DOWN_PULLBACK;
      if(rangePosition <= InpContextImpulseZoneMax && (closePrice > emaFast || retraceRatio > 0.382))
...
bool BuildContextPhase(ContextPhaseContext &ctx)
  {
   ...
   ctx.tierALongEligible = (ctx.phase == CTX_UP_PULLBACK &&
                            ctx.activeWaveRetracement >= InpFibEntryMinRatio &&
                            ctx.activeWaveRetracement <= 0.786 &&
                            close1 >= ctx.emaSlow);
   ctx.tierAShortEligible = (ctx.phase == CTX_DOWN_PULLBACK &&
                             ctx.activeWaveRetracement >= InpFibEntryMinRatio &&
                             ctx.activeWaveRetracement <= 0.786 &&
                             close1 <= ctx.emaSlow);
   ctx.tierBLongEligible = ((ctx.phase == CTX_UP_PULLBACK || ctx.phase == CTX_UP_IMPULSE) &&
                            ctx.activeWaveRetracement >= 0.236 &&
                            ctx.activeWaveRetracement <= 0.786);
   ctx.tierBShortEligible = ((ctx.phase == CTX_DOWN_PULLBACK || ctx.phase == CTX_DOWN_IMPULSE) &&
                             ctx.activeWaveRetracement >= 0.236 &&
                             ctx.activeWaveRetracement <= 0.786);
   ctx.valid = true;
   stageContextValidCount++;
   if(ctx.tierALongEligible)
      stageTierALongEligibleCount++;
   if(ctx.tierBLongEligible)
      stageTierBLongEligibleCount++;
   if(ctx.tierAShortEligible)
      stageTierAShortEligibleCount++;
   if(ctx.tierBShortEligible)
      stageTierBShortEligibleCount++;
   return true;
  }

bool BuildPullbackStructure(PivotPoint &pivots[], PatternStructureContext &structure)
  {
   ...
   structure.sweepFloor = MathMax(PipsToPrice(InpTierBMinPullbackPips),
                                  structure.patternAtr * InpPullbackATRMultiple);
   structure.structureFloor = MathMax(PipsToPrice(InpMinContinuationPips),
                                      structure.patternAtr * InpMinContinuationATR);
   ...
   structure.failureGap = MathMax(PipsToPrice(InpMinPivotHoldGapPips),
                                  structure.patternAtr * InpPivotHoldToleranceATR);
   ...
   stagePullbackStructureBuiltCount++;
   return true;
  }
...
string PullbackDepthBucket(double ratio)
  {
   if(ratio < 0.382)
      return "shallow";
   if(ratio <= 0.618)
...
double ComputePullbackRetracementRatio(int direction,
                                       const ContextPhaseContext &ctx,
                                       double pivotPrice)
  {
   double range = ctx.activeWaveHigh - ctx.activeWaveLow;
   if(range <= 0.0)
      return 0.0;

   double ratio = 0.0;
   if(direction > 0)
      ratio = (ctx.activeWaveHigh - pivotPrice) / range;
   else
      ratio = (pivotPrice - ctx.activeWaveLow) / range;
...
bool PassFibEntryFilter(double ratio, bool strict)
  {
...
string BuildBullishPullbackType(bool higherLowFormed,
                                bool reclaimConfirmed,
                                bool retestContinuation)
  {
   if(retestContinuation)
      return "retest_continuation";
   if(higherLowFormed && reclaimConfirmed)
      return "higher_low_reclaim";
   if(reclaimConfirmed)
      return "pullback_reclaim";
   if(higherLowFormed)
      return "higher_low_candidate";
   return "pullback_only";
  }

string BuildBearishPullbackType(bool lowerHighFormed,
                                bool rejectConfirmed,
                                bool retestContinuation)
  {
   if(retestContinuation)
      return "retest_continuation";
   if(lowerHighFormed && rejectConfirmed)
      return "lower_high_reject";
   if(rejectConfirmed)
      return "pullback_reject";
   if(lowerHighFormed)
      return "lower_high_candidate";
   return "pullback_only";
  }

string BuildBullishPatternStateLabel(bool inFibZone,
                                     bool higherLowFormed,
                                     bool reclaimConfirmed,
                                     bool continuationBreak)
  {
   if(continuationBreak)
      return PatternStateLabel(PATTERN_CONTINUATION_BREAK_READY_BULL);
   if(reclaimConfirmed)
      return PatternStateLabel(PATTERN_BULLISH_RECLAIM_READY);
   if(higherLowFormed)
      return PatternStateLabel(PATTERN_HIGHER_LOW_CANDIDATE);
   if(inFibZone)
      return PatternStateLabel(PATTERN_FIB_ZONE_REACHED_BULL);
   return PatternStateLabel(PATTERN_PULLBACK_IN_PROGRESS_BULL);
  }

string BuildBearishPatternStateLabel(bool inFibZone,
                                     bool lowerHighFormed,
                                     bool rejectConfirmed,
                                     bool continuationBreak)
  {
   if(continuationBreak)
      return PatternStateLabel(PATTERN_CONTINUATION_BREAK_READY_BEAR);
   if(rejectConfirmed)
      return PatternStateLabel(PATTERN_BEARISH_REJECT_READY);
   if(lowerHighFormed)
      return PatternStateLabel(PATTERN_LOWER_HIGH_CANDIDATE);
   if(inFibZone)
      return PatternStateLabel(PATTERN_FIB_ZONE_REACHED_BEAR);
   return PatternStateLabel(PATTERN_PULLBACK_IN_PROGRESS_BEAR);
  }

bool FillBullishContinuationSetup(const ContextPhaseContext &ctx,
                                  const PatternStructureContext &structure,
                                  const PivotPoint &pivots[],
                                  const string tier,
                                  ContinuationSetup &setup)
  {
   ResetContinuationSetup(setup);

   int parentShift = iBarShift(runtimeSymbol, InpPatternTimeframe, ctx.latestHigh.time, false);
   if(parentShift < 3)
      return false;

   int searchCount = MathMin(parentShift, InpPatternScanBars - 1);
   if(searchCount < 3)
      return false;

   int pullbackShift = iLowest(runtimeSymbol, InpPatternTimeframe, MODE_LOW, searchCount, 1);
   if(pullbackShift < 1 || pullbackShift >= parentShift)
      return false;

   double pullbackPrice = iLow(runtimeSymbol, InpPatternTimeframe, pullbackShift);
   datetime pullbackTime = iTime(runtimeSymbol, InpPatternTimeframe, pullbackShift);
   if(pullbackPrice <= 0.0 || pullbackTime <= 0)
      return false;

   double pullbackDepthPips = (ctx.latestHigh.price - pullbackPrice) / GetPipSize();
   double pullbackRatio = ComputePullbackRetracementRatio(1, ctx, pullbackPrice);
   double minPullbackPips = (tier == "tier_a") ? InpTierAMinPullbackPips : InpTierBMinPullbackPips;
   if(pullbackDepthPips < MathMax(minPullbackPips, structure.sweepFloor / GetPipSize()))
      return false;
   if(!PassFibEntryFilter(pullbackRatio, tier == "tier_a"))
      return false;
   stageFibFilterPassCount++;

   PivotPoint latestLow;
   PivotPoint previousLow;
   PivotPoint latestHigh;
   PivotPoint previousHigh;
   FindLatestTypePivots(pivots, false, latestLow, previousLow);
   FindLatestTypePivots(pivots, true, latestHigh, previousHigh);

   bool higherLowFormed = false;
   PivotPoint higherLowPivot = PivotFromLevel(0.0, 0, false);
   if(latestLow.valid &&
      latestLow.shift < pullbackShift &&
      latestLow.shift <= 8 &&
      latestLow.price > pullbackPrice + structure.failureGap)
     {
      higherLowFormed = true;
      higherLowPivot = latestLow;
      stageHigherLowFormedCount++;
     }

   double reclaimLevel = 0.0;
   int reclaimCount = pullbackShift - 2;
   if(reclaimCount >= 1)
     {
      int reclaimShift = iHighest(runtimeSymbol, InpPatternTimeframe, MODE_HIGH, reclaimCount, 2);
      if(reclaimShift >= 0)
         reclaimLevel = iHigh(runtimeSymbol, InpPatternTimeframe, reclaimShift);
     }
   if(reclaimLevel <= 0.0)
     {
      if(previousHigh.valid && previousHigh.shift < pullbackShift)
         reclaimLevel = previousHigh.price;
      else
         reclaimLevel = iHigh(runtimeSymbol, InpPatternTimeframe, 2);
     }
   if(reclaimLevel <= 0.0)
      return false;

   bool inFibZone = (pullbackRatio >= 0.382 && pullbackRatio <= 0.618);
   bool reclaimConfirmed = (structure.currentClose > reclaimLevel + structure.reclaimBuffer &&
                            structure.currentClose > structure.currentOpen);
   if(reclaimConfirmed)
      stageReclaimConfirmedCount++;
   bool continuationBreak = (structure.currentClose > ctx.latestHigh.price + structure.breakBuffer &&
                             structure.currentClose > structure.currentOpen &&
                             (reclaimConfirmed || higherLowFormed));
   double prevClose = iClose(runtimeSymbol, InpPatternTimeframe, 2);
   bool retestContinuation = (prevClose > ctx.latestHigh.price + structure.breakBuffer &&
                              structure.currentLow <= ctx.latestHigh.price + structure.retestTolerance &&
                              structure.currentClose > ctx.latestHigh.price + structure.breakBuffer &&
                              structure.currentClose > structure.currentOpen);

   double minCloseLocation = (tier == "tier_a") ? InpTierAMinCloseLocation : InpTierBMinCloseLocation;
   if(structure.currentCloseLocation < minCloseLocation)
      return false;
   if(!(higherLowFormed || retestContinuation))
      return false;
   if(!(reclaimConfirmed || continuationBreak || retestContinuation))
      return false;

   double waveHeight = ctx.activeWaveHigh - ctx.activeWaveLow;
   if(waveHeight < structure.structureFloor)
      return false;

   stageSetupValidCount++;
   setup.valid = true;
   setup.direction = 1;
   setup.tier = tier;
   ...
   setup.pullbackTypeLabel = BuildBullishPullbackType(higherLowFormed, reclaimConfirmed, retestContinuation);
   setup.pullbackDepthBucket = PullbackDepthBucket(pullbackRatio);
   setup.patternLabel = "higher_timeframe_swing_continuation";
   setup.patternStateLabel = BuildBullishPatternStateLabel(inFibZone, higherLowFormed, reclaimConfirmed, continuationBreak || retestContinuation);
   setup.volatilityBucket = structure.volatilityBucket;
   setup.referencePivot = ctx.latestHigh;
   setup.sweepPivot = PivotFromLevel(pullbackPrice, pullbackTime, false);
   setup.continuationPivot = higherLowFormed ? higherLowPivot : setup.sweepPivot;
   setup.failurePivot = setup.continuationPivot;
   setup.referenceLevel = ctx.latestHigh.price;
   setup.reclaimLevel = reclaimLevel + structure.breakBuffer;
   setup.retestLevel = ctx.latestHigh.price;
   setup.continuationLevel = ctx.latestHigh.price;
   setup.stopAnchor = higherLowFormed ? higherLowPivot.price : pullbackPrice;
   setup.invalidationLevel = reclaimLevel - PipsToPrice(InpAcceptanceExitBufferPips);
   setup.priorSwingTarget = ctx.priorSwingHigh;
   setup.structureHeight = waveHeight;
   setup.structureHeightPips = waveHeight / GetPipSize();
...
bool FillBearishContinuationSetup(const ContextPhaseContext &ctx,
                                  const PatternStructureContext &structure,
                                  const PivotPoint &pivots[],
                                  const string tier,
                                  ContinuationSetup &setup)
  {
   ...
   int parentShift = iBarShift(runtimeSymbol, InpPatternTimeframe, ctx.latestLow.time, false);
   if(parentShift < 3)
      return false;
   ...
   int pullbackShift = iHighest(runtimeSymbol, InpPatternTimeframe, MODE_HIGH, searchCount, 1);
   if(pullbackShift < 1 || pullbackShift >= parentShift)
      return false;

   double pullbackPrice = iHigh(runtimeSymbol, InpPatternTimeframe, pullbackShift);
   datetime pullbackTime = iTime(runtimeSymbol, InpPatternTimeframe, pullbackShift);
   if(pullbackPrice <= 0.0 || pullbackTime <= 0)
      return false;

   double pullbackDepthPips = (pullbackPrice - ctx.latestLow.price) / GetPipSize();
   double pullbackRatio = ComputePullbackRetracementRatio(-1, ctx, pullbackPrice);
   double minPullbackPips = (tier == "tier_a") ? InpTierAMinPullbackPips : InpTierBMinPullbackPips;
   if(pullbackDepthPips < MathMax(minPullbackPips, structure.sweepFloor / GetPipSize()))
      return false;
   ...
   bool lowerHighFormed = false;
   PivotPoint lowerHighPivot = PivotFromLevel(0.0, 0, true);
   if(latestHigh.valid &&
      latestHigh.shift < pullbackShift &&
      latestHigh.shift <= 8 &&
      latestHigh.price < pullbackPrice - structure.failureGap)
     {
      lowerHighFormed = true;
      lowerHighPivot = latestHigh;
      stageLowerHighFormedCount++;
     }

   double rejectLevel = 0.0;
   int rejectCount = pullbackShift - 2;
   if(rejectCount >= 1)
     {
      int rejectShift = iLowest(runtimeSymbol, InpPatternTimeframe, MODE_LOW, rejectCount, 2);
      if(rejectShift >= 0)
         rejectLevel = iLow(runtimeSymbol, InpPatternTimeframe, rejectShift);
     }
   if(rejectLevel <= 0.0)
     {
      if(previousLow.valid && previousLow.shift < pullbackShift)
         rejectLevel = previousLow.price;
      else
         rejectLevel = iLow(runtimeSymbol, InpPatternTimeframe, 2);
     }
   if(rejectLevel <= 0.0)
      return false;

   bool inFibZone = (pullbackRatio >= 0.382 && pullbackRatio <= 0.618);
   bool rejectConfirmed = (structure.currentClose < rejectLevel - structure.reclaimBuffer &&
                           structure.currentClose < structure.currentOpen);
   if(rejectConfirmed)
      stageReclaimConfirmedCount++;
   bool continuationBreak = (structure.currentClose < ctx.latestLow.price - structure.breakBuffer &&
                             structure.currentClose < structure.currentOpen &&
                             (rejectConfirmed || lowerHighFormed));
   double prevClose = iClose(runtimeSymbol, InpPatternTimeframe, 2);
   bool retestContinuation = (prevClose < ctx.latestLow.price - structure.breakBuffer &&
                              structure.currentHigh >= ctx.latestLow.price - structure.retestTolerance &&
                              structure.currentClose < ctx.latestLow.price - structure.breakBuffer &&
                              structure.currentClose < structure.currentOpen);

   double maxCloseLocation = 1.0 - ((tier == "tier_a") ? InpTierAMinCloseLocation : InpTierBMinCloseLocation);
   if(structure.currentCloseLocation > maxCloseLocation)
      return false;
   if(!(lowerHighFormed || retestContinuation))
      return false;
   if(!(rejectConfirmed || continuationBreak || retestContinuation))
      return false;
   ...
   setup.contextBucket = ctx.contextBucket;
   setup.waveLabel = ctx.waveLabel;
   setup.pullbackTypeLabel = BuildBearishPullbackType(lowerHighFormed, rejectConfirmed, retestContinuation);
   setup.pullbackDepthBucket = PullbackDepthBucket(pullbackRatio);
   setup.patternLabel = "higher_timeframe_swing_continuation";
   setup.patternStateLabel = BuildBearishPatternStateLabel(inFibZone, lowerHighFormed, rejectConfirmed, continuationBreak || retestContinuation);
   setup.volatilityBucket = structure.volatilityBucket;
   setup.referencePivot = ctx.latestLow;
   setup.sweepPivot = PivotFromLevel(pullbackPrice, pullbackTime, true);
   setup.continuationPivot = lowerHighFormed ? lowerHighPivot : setup.sweepPivot;
   setup.failurePivot = setup.continuationPivot;
   setup.referenceLevel = ctx.latestLow.price;
   setup.reclaimLevel = rejectLevel - structure.breakBuffer;
   setup.retestLevel = ctx.latestLow.price;
   setup.continuationLevel = ctx.latestLow.price;
   setup.stopAnchor = lowerHighFormed ? lowerHighPivot.price : pullbackPrice;
   setup.invalidationLevel = rejectLevel + PipsToPrice(InpAcceptanceExitBufferPips);
...
bool DetectBearishContinuationSetup(const ContextPhaseContext &ctx,
                                    const PatternStructureContext &structure,
                                    const PivotPoint &pivots[],
                                    ContinuationSetup &setup)
  {
   ...
   return FillBearishContinuationSetup(ctx, structure, pivots, tier, setup);
  }

bool DetectBullishContinuationSetup(const ContextPhaseContext &ctx,
                                    const PatternStructureContext &structure,
                                    const PivotPoint &pivots[],
                                    ContinuationSetup &setup)
  {
   ...
   return FillBullishContinuationSetup(ctx, structure, pivots, tier, setup);
  }
...
bool PendingSetupExpiredOrInvalidated(PendingExecutionContext &ctx)
  {
   ...
   double tolerance = PipsToPrice(InpMinPivotHoldGapPips);
...
bool BuildExecutionTrigger(PendingExecutionContext &ctx, ExecutionTrigger &trigger)
  {
      ...
      if(ctx.setup.direction < 0)
        {
         if(close1 < ctx.setup.reclaimLevel && close1 < open1)
           {
            trigger.fired = true;
            trigger.triggerLabel = "exec_reject_close_confirm";
   ...
   else if(InpExecutionTriggerMode == EXEC_RETEST_HOLD_OR_REJECT)
     {
      if(!ctx.breakoutSeen)
        {
      ...
      if(ctx.setup.direction < 0)
        {
         if(high1 >= ctx.setup.reclaimLevel - tolerance &&
        ...
        {
         if(low1 <= ctx.setup.reclaimLevel + tolerance &&
            close1 > ctx.setup.reclaimLevel &&
            close1 > open1)
           {
            trigger.fired = true;
   ...
   stageTriggerFiredCount++;
   trigger.setupToEntryPips = setupToEntryPips;
   trigger.barsFromSetupToEntry = barsFromSetup;
   return true;
  }

double SelectPartialTarget(const ContinuationSetup &setup, double entry, string &label)
  {
...
double SelectFinalTarget(const ContinuationSetup &setup,
                         double entry,
                         double risk,
                         string &label)
  {
...
bool BuildEntryPlan(const ContinuationSetup &setup,
                    ...
                    EntryPlan &plan)
  {
   ...
   plan.pullbackTypeLabel = setup.pullbackTypeLabel;
   plan.pullbackDepthBucket = setup.pullbackDepthBucket;
   ...
   plan.reason = "htf_swing_cont_" + setup.tier + "_" + setup.sideLabel + "_" + trigger.triggerLabel;
   return true;
  }
...
string StageCountFileName()
  {
   string baseName = runtimeTelemetryFileName;
   int marker = StringFind(baseName, ".csv");
   if(marker >= 0)
      baseName = StringSubstr(baseName, 0, marker);
   return baseName + "_stage_counts.csv";
  }

void WriteDiagnosticStageCountFile()
  {
   int handle = FileOpen(StageCountFileName(),
                         FILE_CSV | FILE_WRITE | FILE_COMMON | FILE_ANSI,
                         ';');
...
bool HasManagedPositionInDirection(int direction)
  {
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      string symbol = PositionGetSymbol(i);
      ...
      if(direction > 0 && positionType == POSITION_TYPE_BUY)
         return true;
      if(direction < 0 && positionType == POSITION_TYPE_SELL)
         return true;
     }
   return false;
  }

double CalculateManagedOpenRiskAmount()
  {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(entryPrice <= 0.0 || stopPrice <= 0.0 || volume <= 0.0)
         continue;

      totalRiskAmount += CalculateRiskAmountByVolume(entryPrice, stopPrice, volume);
     }
   return totalRiskAmount;
  }

double CurrentManagedRiskPercent()
  {
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   if(equity <= 0.0)
      return 0.0;
   return (CalculateManagedOpenRiskAmount() / equity) * 100.0;
  }

bool PassReentryRules(int direction)
  {
   if(!InpAllowSameDirectionReentry && HasManagedPositionInDirection(direction))
      return false;

   if(InpReentryCooldownBars <= 0)
      return true;

   datetime lastEntryTime = (direction > 0) ? lastLongEntryTime : lastShortEntryTime;
   if(lastEntryTime <= 0)
      return true;

   int barsSinceLastEntry = BarsBetweenTimes(lastEntryTime, TimeCurrent(), InpExecutionTimeframe);
   if(barsSinceLastEntry < 0)
      return true;
   return (barsSinceLastEntry >= InpReentryCooldownBars);
  }

bool CanAddManagedPosition(const EntryPlan &plan, double additionalRiskAmount)
  {
   if(CountManagedPositions() >= InpMaxManagedPositions)
      return false;
   if(!PassReentryRules(plan.direction))
      return false;
   ...
   double totalRiskPercent = ((CalculateManagedOpenRiskAmount() + additionalRiskAmount) / equity) * 100.0;
   return (totalRiskPercent <= InpMaxTotalRiskPercent + 1e-8);
  }
...
bool PassGlobalGuards()
  {
   ...
   if(CountManagedPositions() >= InpMaxManagedPositions)
      return false;
   if(CurrentManagedRiskPercent() >= InpMaxTotalRiskPercent)
      return false;
   return true;
  }
...
void StagePendingExecution(const ContinuationSetup &setup)
  {
...
bool SelectPreferredSetup(const ContinuationSetup &shortSetup,
                          const ContinuationSetup &longSetup,
                          ContinuationSetup &selected)
  {
   ResetContinuationSetup(selected);
...
bool ExecuteEntry(const EntryPlan &plan)
  {
   ...
   double additionalRiskAmount = CalculateRiskAmountByVolume(plan.entry, plan.stop, volume);
   if(additionalRiskAmount <= 0.0)
      return false;
   if(!CanAddManagedPosition(plan, additionalRiskAmount))
      return false;

   EntryPlan localPlan = plan;
   localPlan.plannedRiskAmount = additionalRiskAmount;
...
void ProcessNewPatternBar()
  {
   if(hasPendingPlan || CountManagedPositions() >= InpMaxManagedPositions)
      return;
   if(!PassGlobalGuards())
     {
      ResetPendingExecution(pendingExecution);
   ...
   if(!BuildPullbackStructure(pivots, structure))
     {
      ResetPendingExecution(pendingExecution);
      return;
     }

   ContinuationSetup shortSetup;
   ContinuationSetup longSetup;
   ContinuationSetup selected;
   DetectBearishContinuationSetup(ctx, structure, pivots, shortSetup);
   DetectBullishContinuationSetup(ctx, structure, pivots, longSetup);
      ...
      if(hasPendingPlan)
        {
         ...
         if(activePlan.direction > 0)
            lastLongEntryTime = dealTime;
         else if(activePlan.direction < 0)
            lastShortEntryTime = dealTime;
...
bool IsAllowedContextTimeframe(ENUM_TIMEFRAMES tf)
  {
   return (tf == PERIOD_M30 || tf == PERIOD_H1);
  }

bool IsAllowedPatternTimeframe(ENUM_TIMEFRAMES tf)
  {
   return (tf == PERIOD_M10 || tf == PERIOD_M15 || tf == PERIOD_M30);
  }
...
int OnInit()
  {
   ...
   if(InpMagicNumber <= 0 || InpContextPivotSpan <= 0 || InpPatternPivotSpan <= 0 || InpExecutionPivotSpan <= 0 ||
      ...
      InpPullbackATRMultiple <= 0.0 || InpTierAMinPullbackPips <= 0.0 || InpTierBMinPullbackPips <= 0.0 ||
      InpReclaimBufferPips <= 0.0 || InpPivotHoldToleranceATR <= 0.0 || InpMinPivotHoldGapPips <= 0.0 ||
      InpRiskPercent <= 0.0 || InpTargetRMultiple <= 0.0 ||
      InpMaxManagedPositions <= 0 || InpReentryCooldownBars < 0 || InpMaxTotalRiskPercent <= 0.0 ||
      InpContextImpulseZoneMin < 0.0 || InpContextImpulseZoneMin > 1.0 ||
      InpContextImpulseZoneMax < 0.0 || InpContextImpulseZoneMax > 1.0 ||
...
void OnDeinit(const int)
  {
   LogDiagnosticStageCounts();
   WriteDiagnosticStageCountFile();
