...
bool TSConfigValid(const TickShockConfig &config)
  {
   return config.grid_ms>0 && config.baseline_minutes>0 &&
          ...
          config.min_robust_z>0.0 && config.min_efficiency>0.0 &&
          config.min_move_spread_ratio>0.0 && config.min_tick_intensity_ratio>0.0 &&
          config.max_spread_median_ratio>0.0 && config.max_quote_age_ms>=0 &&
          config.burst_quiet_ms>0 && config.burst_max_ms>=config.burst_quiet_ms &&
          config.pullback_min_pct>=0.0 && config.pullback_max_pct>=config.pullback_min_pct &&
          config.continuation_invalid_pct>config.pullback_max_pct &&
