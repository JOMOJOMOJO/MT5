...
#define TS15K_MAX_HORIZON_LAG_MS 30000
...
struct TickShock15JRecord
  {
   bool active;bool entered;bool complete;bool write_pending;bool written;bool invalid;bool censored;bool censored_horizon_lag;bool feature_valid;
   ...
   long horizon_target_msc[TS15J_HORIZONS];long horizon_quote_msc[TS15J_HORIZONS];long horizon_lag_ms[TS15J_HORIZONS];bool horizon_done[TS15J_HORIZONS];bool horizon_censored[TS15J_HORIZONS];int censored_horizon_index;
...
struct TickShock15JPool
  {TickShock15JRecord records[TS15J_POOL_CAPACITY];long armed;long completed;long censored;long horizon_lag_censored;long capacity_hits;long invalid_paths;};

string TS15JSchema(){return "tickshock-post-shock-excursion-v2";}
void TS15JResetRecord(TickShock15JRecord &r){ZeroMemory(r);r.censored_horizon_index=-1;r.existing_source=TS15G_RISK_INVALID;r.existing_continuation_result=TS15G_PENDING;r.existing_reversal_result=TS15G_PENDING;for(int i=0;i<TS15J_DISTANCES;++i){r.continuation_hit_ms[i]=-1;r.reversal_hit_ms[i]=-1;}}
...
bool TS15JArm(TickShock15JPool &pool,const string episode_id,const string event_id,const string symbol,const long cluster_id,const int direction,
              ...
              const double tick_size,const double broker_stop_distance)
  {
   ...
   for(int h=0;h<TS15J_HORIZONS;++h)r.horizon_target_msc[h]=r.t0_msc+(long)TS15J_HORIZON_SECONDS[h]*1000;
...
void TS15JProcessQuote(TickShock15JRecord &r,const long quote_msc,const long processing_msc,const double bid,const double ask,const bool fallback)
  {
   ...
   for(int h=0;h<TS15J_HORIZONS;++h)
     if(!r.horizon_done[h]&&quote_msc>=r.horizon_target_msc[h]&&quote_msc-r.horizon_target_msc[h]>TS15K_MAX_HORIZON_LAG_MS)
       {
        r.horizon_quote_msc[h]=quote_msc;r.horizon_lag_ms[h]=quote_msc-r.horizon_target_msc[h];r.horizon_censored[h]=true;
        r.censored=true;r.censored_horizon_lag=true;r.censored_horizon_index=h;r.complete=true;r.active=false;r.write_pending=true;return;
       }
   ...
   for(int h=0;h<TS15J_HORIZONS;++h)if(!r.horizon_done[h]&&quote_msc>=r.horizon_target_msc[h])
     {r.horizon_done[h]=true;r.horizon_quote_msc[h]=quote_msc;r.horizon_lag_ms[h]=quote_msc-r.horizon_target_msc[h];r.continuation_mfe_h[h]=r.continuation_mfe;r.continuation_mae_h[h]=r.continuation_mae;r.reversal_mfe_h[h]=r.reversal_mfe;r.reversal_mae_h[h]=r.reversal_mae;}
