...
bool TS15FBuildFeatures(const TickShock15FBarState &state,const long target_msc,const long quote_msc,const long processing_msc,const double bid,const double ask,const int shock_direction,const double severity,const double initial_shock,const double anchor_spread,const long confirmation_delay_ms,const long repeat_count,const long same_repeats,const long opposite_repeats,const bool origin_recross,const double pre_m1_rms,const double usd_factor,const int usd_sign,const int usd_pairs,TickShock15FFeatureSnapshot &s)
  {
   ...
   TickShock15FBar m1[],m5[],m15[];TS15FChronologicalBars(state,m1);TS15FAggregateBars(state,5,m5);TS15FAggregateBars(state,15,m15);double mid=(bid+ask)*.5,spread=ask-bid,atr1=0,atr5=0,atr15=0,e20=0,e50=0,e20_3=0,e50_3=0,r1=0,r5=0,r15=0,rv1=0,rv5=0,rv15=0,pos=0,vol_pct=0;bool a1=TS15FATRFromBars(m1,14,atr1),a5=TS15FATRFromBars(m5,14,atr5),a15=TS15FATRFromBars(m15,14,atr15),v20=TS15FEMA(m1,20,0,e20),v50=TS15FEMA(m1,50,0,e50),v203=TS15FEMA(m1,20,3,e20_3),v503=TS15FEMA(m1,50,3,e50_3),vr1=TS15FTrailingReturn(m1,1,r1),vr5=TS15FTrailingReturn(m1,5,r5),vr15=TS15FTrailingReturn(m1,15,r15),vv1=TS15FRealizedVol(m1,1,rv1),vv5=TS15FRealizedVol(m1,5,rv5),vv15=TS15FRealizedVol(m1,15,rv15),vp=TS15FDailyPosition(state,mid,pos),vpercent=TS15FVolatilityPercentile(m1,15,128,vol_pct);double m5e20=0,m5e50=0,m15e20=0,m15e50=0;bool m5a=TS15FEMA(m5,20,0,m5e20)&&TS15FEMA(m5,50,0,m5e50),m15a=TS15FEMA(m15,20,0,m15e20)&&TS15FEMA(m15,50,0,m15e50),ok=false;
   TS15FSet(s,0,TS15FNormalize(mid-e20,atr1,ok),v20&&a1&&ok);TS15FSet(s,1,TS15FNormalize(mid-e50,atr1,ok),v50&&a1&&ok);TS15FSet(s,2,TS15FNormalize(e20-e20_3,atr1,ok),v20&&v203&&a1&&ok);TS15FSet(s,3,TS15FNormalize(e50-e50_3,atr1,ok),v50&&v503&&a1&&ok);double al1=v20&&v50?(e20>e50?1:(e20<e50?-1:0)):0,al5=m5a?(m5e20>m5e50?1:(m5e20<m5e50?-1:0)):0,al15=m15a?(m15e20>m15e50?1:(m15e20<m15e50?-1:0)):0;TS15FSet(s,4,al1,v20&&v50);TS15FSet(s,5,al5,m5a);TS15FSet(s,6,al15,m15a);TS15FSet(s,7,al5*shock_direction,m5a&&shock_direction!=0);TS15FSet(s,8,al15*shock_direction,m15a&&shock_direction!=0);TS15FSet(s,9,r1,vr1);TS15FSet(s,10,r5,vr5);TS15FSet(s,11,r15,vr15);TS15FSet(s,12,r5*shock_direction,vr5&&shock_direction!=0);
   double eff=0;if(ArraySize(m1)>15){double den=0;for(int i=ArraySize(m1)-15;i<ArraySize(m1);++i)den+=MathAbs(m1[i].close-m1[i-1].close);eff=den>0?MathAbs(m1[ArraySize(m1)-1].close-m1[ArraySize(m1)-16].close)/den:0;}TS15FSet(s,13,eff,ArraySize(m1)>15);TS15FSet(s,14,pos,vp);TS15FSet(s,15,atr1,a1);TS15FSet(s,16,atr5,a5);TS15FSet(s,17,atr15,a15);TS15FSet(s,18,rv1,vv1);TS15FSet(s,19,rv5,vv5);TS15FSet(s,20,rv15,vv15);TS15FSet(s,21,TS15FNormalize(rv1,rv15,ok),vv1&&vv15&&ok);TS15FSet(s,22,TS15FNormalize(initial_shock,pre_m1_rms*mid,ok),pre_m1_rms>0&&ok);TS15FSet(s,23,vol_pct,vpercent);TS15FSet(s,24,spread,true);TS15FSet(s,25,TS15FNormalize(spread,atr1,ok),a1&&ok);TS15FSet(s,26,TS15FNormalize(spread,pre_m1_rms*mid,ok),pre_m1_rms>0&&ok);TS15FSet(s,27,ArraySize(m1)>0?m1[ArraySize(m1)-1].updates:0,ArraySize(m1)>0);TS15FSet(s,28,processing_msc-quote_msc,true);TS15FSet(s,29,severity,true);TS15FSet(s,30,TS15FNormalize(initial_shock,anchor_spread,ok),ok);TS15FSet(s,31,confirmation_delay_ms,confirmation_delay_ms>=0);TS15FSet(s,32,repeat_count,true);long repeat_denominator=repeat_count>0?repeat_count:1;TS15FSet(s,33,(double)(same_repeats-opposite_repeats)/(double)repeat_denominator,true);TS15FSet(s,34,origin_recross?1:0,true);double usd_alignment=usd_factor>0?1:(usd_factor<0?-1:0);TS15FSet(s,35,usd_alignment*(double)usd_sign*(double)shock_direction,usd_pairs>=3&&usd_sign!=0&&shock_direction!=0);s.valid=true;for(int i=0;i<TS15F_FEATURES;++i)if(!s.available[i]){s.valid=false;break;}s.reason=s.valid?"AVAILABLE":"FEATURE_UNAVAILABLE";return s.valid;
  }

void TS15FObserveControl(TickShockContextFeatureState &s,const string run_id,const string symbol,const long quote_msc,const long processing_msc,const double bid,const double ask,const bool shock_active,const double usd_factor,const int usd_pairs)
  {
   ...
   long grid=(quote_msc/TS15F_CONTROL_GRID_MS)*TS15F_CONTROL_GRID_MS;if(shock_active||quote_msc-grid>1000||grid<=0)return;TickShock15FBar bars[];TS15FChronologicalBars(s.bars,bars);double r5=0;if(ArraySize(bars)<750||!TS15FTrailingReturn(bars,5,r5)||r5==0)return;TS15FResetControl(c);c.active=true;c.control_id=StringFormat("%s_%s_ctrl_%I64d",run_id,symbol,++s.control_sequence);c.symbol=symbol;c.anchor_msc=grid;c.processing_msc=processing_msc;c.pseudo_direction=r5>0?1:-1;c.anchor_bid=bid;c.anchor_ask=ask;c.reason="ACTIVE";s.control=c;
  }
