...
$expertTarget = Join-Path $portable "MQL5\Experts\dev\mql\Experts"
$presetTarget = Join-Path $portable "MQL5\Profiles\Tester"
New-Item -ItemType Directory -Force -Path $expertTarget, $presetTarget | Out-Null
Copy-Item -LiteralPath (Join-Path $RepoRoot "mql\Experts\ExpectedValue_LongOnly_BucketLab.ex5") -Destination (Join-Path $expertTarget "ExpectedValue_LongOnly_BucketLab.ex5") -Force

$variants = @(
    @{Name="v2_4_h30_r135_cd6"; Hold="30"; R="1.35"; Cool="6"; Magic="2026052102"},
    @{Name="v2_4_h30_r125_cd6"; Hold="30"; R="1.25"; Cool="6"; Magic="2026052103"},
    @{Name="v2_4_h25_r125_cd6"; Hold="25"; R="1.25"; Cool="6"; Magic="2026052104"},
    @{Name="v2_4_h30_r125_cd4"; Hold="30"; R="1.25"; Cool="4"; Magic="2026052105"},
    @{Name="v2_4_h25_r120_cd4"; Hold="25"; R="1.20"; Cool="4"; Magic="2026052106"},
    @{Name="v2_4_h20_r120_cd4"; Hold="20"; R="1.20"; Cool="4"; Magic="2026052107"}
)
...
foreach($variant in $variants) {
    ...
    Set-PresetValue $lines "InpPresetName" "ExpectedValue_LongOnly_BucketLab_$($variant.Name)_2025"
    ...
    $variantDir = Join-Path $runRoot $variant.Name
    New-Item -ItemType Directory -Force -Path $variantDir, (Join-Path $variantDir "raw"), (Join-Path $variantDir "ini") | Out-Null
...
"@ | Set-Content -LiteralPath $iniPath -Encoding ASCII
    ...
    Copy-Item -LiteralPath (Join-Path $portable "$reportName.htm") -Destination (Join-Path $variantDir "$reportName.htm") -Force
    Get-ChildItem -Path $portable -Filter "$reportName*.png" | Copy-Item -Destination $variantDir -Force
...
$results | Export-Csv -LiteralPath (Join-Path $runRoot "sweep_summary.csv") -NoTypeInformation
